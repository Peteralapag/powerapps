...
$show_limit = isset($_SESSION['BRANCH_CART_LIMIT']) ? $_SESSION['BRANCH_CART_LIMIT'] : 25;
if(isset($_SESSION['BRANCH_CART_BRANCH']))
{
	$branch = $_SESSION['BRANCH_CART_BRANCH'];
} else {
	$branch = $_SESSION['branch_branch'];
}

if(isset($_SESSION['BRANCH_CART_RECIPIENT']))
{
	$recipient = $_SESSION['BRANCH_CART_RECIPIENT'];
} else {
	$recipient = '';
}
?>
...
function load_data()
{
	...
	$.post("./Modules/" + module + "/includes/create_order_cart_data.php", { page: page, branch: branch, limit: limit, recipient: recipient },
	function(data) {
		$('#pagesdata').html(data);
		rms_reloaderOff();
	});
...
function performSearch()
{
    ...
    $.post("./Modules/" + module + "/includes/create_order_cart_data.php", 
...
$(document).ready(function()
{
	...
	document.querySelectorAll('.pagination-btn').forEach(button => {
	    button.addEventListener('click', function () {
	        ...
	        var limit = $('#limit').val();
	        var recipient = $('#recipient').val();
	        $.post('./Modules/' + module + '/includes/create_order_cart_data.php', { limit: limit, page: page, recipient: recipient }, function (data) {
	            $('#pagesdata').html(data);
	        });
	    });
	})
...
function loadPage(page)
{
	...
	var recipient = $('#recipient').val();
	rms_reloaderOn('Loading...');
    $.post('./Modules/' + module + '/includes/create_order_cart_data.php', { page: page, limit: limit, recipient: recipient }, function(data) {	     
        $('#pagesdata').html(data);       
    });
}
</script>