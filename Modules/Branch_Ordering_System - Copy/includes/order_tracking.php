...
$show_limit = isset($_SESSION['BRANCH_TRACKER_LIMIT']) ? $_SESSION['BRANCH_TRACKER_LIMIT'] : 25;

if(isset($_SESSION['BRANCH_TRACKER_BRANCH']))
{
	$branch = $_SESSION['BRANCH_TRACKER_BRANCH'];
} else {
	$branch = $_SESSION['branch_branch'];
}

$userlevel = $_SESSION['branch_userlevel'];
if(isset($_SESSION['BRANCH_TRACKER_RECIPIENT']))
{
	$recipient = $_SESSION['BRANCH_TRACKER_RECIPIENT'];
} else {
	$recipient = '';
}
?>
...
function performSearch()
{
	...
	$.post("./Modules/" + module + "/includes/order_tracking_data.php", { search: search, branch: branch, limit: limit, recipient: recipient, orderstatus: orderstatus },
	function(data) {
		$('#pagesdata').html(data);
		rms_reloaderOff();
	});		
...
$(document).ready(function ()
{
	...
	document.querySelectorAll('.pagination-btn').forEach(button => {
	    button.addEventListener('click', function () {
	        ...
	        $.post('./Modules/' + module + '/includes/order_tracking_data.php', { limit: limit, branch: branch, page: page, recipient: recipient }, function (data) {
	            $('#pagesdata').html(data);
	        });
	    });
	})
...
function loadPage(page)
{
    ...
    $.post('./Modules/' + module + '/includes/order_tracking_data.php', { page: page, branch: branch, limit: limit, recipient: recipient }, function(data) {	     
        $('#pagesdata').html(data);       
    });
}
function load_data()
{
	...
	var recipient = $('#recipient').val();
	var page = 1;
	$.post("./Modules/" + module + "/includes/order_tracking_data.php", { page: page, branch: branch, limit: limit, recipient: recipient },
	function(data) {
		$('#pagesdata').html(data);
	});
}
