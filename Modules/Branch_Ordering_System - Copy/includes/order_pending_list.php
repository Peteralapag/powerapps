...
<table style="width: 100%" class="table table-bordered table-hover open-table">
	...
	<tbody>
<?php
	$OPENQUERY = "SELECT request_id,branch,control_no,recipient,created_by,trans_date,order_type FROM wms_order_request WHERE $kwiri status='In-Transit'";
...
?>	
		<tr>
			...
			<td style="padding:1px !important">
				<button class="btn btn-primary btn-sm" onclick="openPending('<?php echo $control_no?>','<?php echo $order_type?>','<?php echo $rowid?>')"><i class="fa-solid fa-circle-info"></i>&nbsp; Details</button>
			</td>
		</tr>
<?php } } else {?>		
		<tr>
...
<script>
function openPending(control_no, order_type, rowid)
{
	var querymode = 'openpending';
	$('#formodalsmtitle').html("TRACKING - ORDER DETAILS [[ <strong>" + control_no + "</strong> ]]");
	$.post("./Modules/Branch_Ordering_System/apps/form_order_tracking_data.php", { querymode: querymode, rowid: rowid, order_type: order_type, control_no: control_no },
	function(data) {		
