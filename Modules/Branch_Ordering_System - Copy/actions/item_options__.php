...
if(isset($_POST['search']) && !empty($_POST['search'])) {
    $search = $_POST['search'];
    $sqlQuery = "SELECT * FROM wms_itemlist WHERE (item_description LIKE ? OR item_code LIKE ?) AND recipient = ? AND active= ?";
} else {
    $sqlQuery = "SELECT * FROM wms_itemlist WHERE recipient = ?";
}
