...
$partialpending = $function->selectViaControlnoAngBranch($control_no, $branch, $db);
...
$statpDL = $partialpending > 0? 'bg-danger': '';

?>
<div style="padding:10px">
	<div class="mrs-wrappers" style="margin:0 auto;margin-top:10px">
		<span class="psamouse" onclick="partialreceiving('<?php echo $control_no?>','<?php echo $branch?>')">
			<i class="fa fa-list" aria-hidden="true"></i> Partial Receiving
			<span class="badge <?php echo $statpDL?>">
				<?php echo $partialpending?>
			</span>
		</span>
...
<div class="mrs-wrappers" style="margin:0 auto">	
	<table style="width: 100%;font-family:sans-serif" class="table">
		<tr class="table-thth">
			<th colspan="7">MATERIAL REQUISITION FORMss</th>
		</tr>
		<tr>
...
<!--div class="mrs-wrappers" style="margin:0 auto;margin-top:10px;text-align:center">
	<button <?php echo $rcvdel; ?> class="btn btn-primary" onclick="receivedDelivery('<?php echo $control_no; ?>')">Delivery 
	Complete Received</button>
</div-->
<?php } mysqli_close($db); ?>
...
function partialreceiving(controlno,branch){
	
	var module = '<?php echo MODULE_NAME; ?>';

	$('#modaltitle').html("PARTIAL RECEIVING");
	$.post("./Modules/" + module + "/apps/partial_receiving.php", { controlno: controlno, branch: branch },
	function(data) {		
