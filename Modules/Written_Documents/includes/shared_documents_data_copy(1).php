...
if($user_level >= 80)
{
	...
	{
		$search = $_POST['search'];
		$q = "WHERE file_name LIKE '%$search%' AND company='$company' AND shared_to='$organization' AND subfolder='$subfolder' AND  username='$username'";
	} else { 
		$q = "WHERE company='$company' AND shared_to='$organization' AND subfolder='$subfolder' AND  username='$username'"; 
	}
}
?>
	...
	<?php
		...
		if ( $RESULTS->num_rows > 0 ) 
		{
			...
			while($ROWS = mysqli_fetch_array($RESULTS))  
			{
				...
				$date_uploaded = date("M m, Y @ h:i A", strtotime($ROWS['date_uploaded']));
