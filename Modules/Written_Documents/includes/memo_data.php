...
if($user_level >= 10)
{
	...
	{
		$search = $_POST['search'];
		$q = "WHERE file_name LIKE '%$search%' AND organization='$organization' AND subfolder='$subfolder'"; 		
	} else {
		$q = "WHERE organization='$organization' AND subfolder='$subfolder'"; 
	}
} else {
	$sqlPriviledge = "SELECT * FROM tbl_system_permission WHERE username='$username'";
	$previledgeResult=mysqli_query($db,$sqlPriviledge);    
	if ( $previledgeResult->num_rows > 0 )
	{
		if(isset($_POST['search']))
		{
			$search = $_POST['search'];
			$q = "WHERE file_name LIKE '%$search%' AND company='$company' AND organization='$organization' AND subfolder='$subfolder'"; 
		} else {
			$q = "WHERE company='$company' AND organization='$organization' AND subfolder='$subfolder'"; 
		}
	} 
	else
	{ 
		echo "Un-Authorized access is not allowed";
		exit();			
	}
}
?>
<div class="tableFixHead ">
	<table id="filedata" style="width:100%" class="table table-hover table-striped table-bordered">
		...
		<tbody>		
<?php
	$SQL = "SELECT * FROM tbl_archived_memo $q";
	...
	?>
				<tr ondblclick="openThisFile('<?php echo $file_id; ?>','<?php echo $file_name; ?>','<?php echo $organization; ?>')">
