...
{
	$rowid = $_POST['rowid'];
	$QUERY = "SELECT * FROM fds_itemlist WHERE id='$rowid'";
	...
	<table style="width: 100%" class="table">
		...
		<tr>
			...
			<td>
				...
				<input id="recipient" type="text" class="form-control" value="<?php echo $recipient ?>" disabled>
			</td>
		</tr>
		<tr>
			<th>Item Loc.</th>
			<td>
				...
				<input id="item_location" type="text" class="form-control" value="<?php echo $item_location?>" disabled>
			</td>
		</tr>
		<tr>
			<th>Item Code</th>
...
function deleteItemYes(params)
{
	...
	{
		var limit = $('#limit').val();
		$.post("./Modules/Frozen_Dough_Management/actions/deleteitem_process.php", { rowid: params,limit: limit },
		function(data) {		
			$('.results').html(data);
			rms_reloaderOff();
		});
...
/*	if(supplier === '')
	...
	setTimeout(function()
	{
		$.post("./Modules/Frozen_Dough_Management/actions/itemlist_process.php",
		{ 
			mode: mode,
			rowid: rowid,
			active: active, 
