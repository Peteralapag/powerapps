...
$sqlQuery = "SELECT * FROM fds_order_request WHERE request_id='$rowid'";
...
{
	$editid = $_POST['editid'];
	$sqlQueryEdit = "SELECT * FROM fds_branch_order WHERE id='$editid'";
	...
	$sqlQueryEditRemarks = "SELECT * FROM fds_branch_order_remarks WHERE branch='$branch' AND control_no='$mrs_no'";
...
//	var remarks = $('#remarks').val();
	...
	setTimeout(function()
	{
		$.post("./Modules/Frozen_Dough_Management/actions/create_order_process.php",
		{
			mode: mode,
			editid: editid,
			rowid: rowid,
...
function saveRemarks(params)
{
	...
	setTimeout(function()
	{
		$.post("./Modules/Frozen_Dough_Management/actions/save_order_remarks.php",
		{
			mode: mode,
			rowid: rowid,
			branch: branch,
...
{
	$.post("./Modules/Frozen_Dough_Management/includes/request_details_data.php", { rowid: rowid  },
	function(data) {		
		$('#smnavdata').html(data);
	});	
}
