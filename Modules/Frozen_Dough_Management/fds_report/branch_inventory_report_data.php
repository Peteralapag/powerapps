...
<table style="width:100%" class="table table-bordered table-striped" id="highLight">
	...
	<tbody>
<?php
	$sqlQuery = "SELECT * FROM fds_itemlist WHERE recipient='$recipient'";
...
?>	
		...
		<?php
			$total = 0;
			$positiveTotal = 0;			
//			$sqlQueryIR = "SELECT * FROM fds_branch_order WHERE DATE_FORMAT(delivery_date, '%Y-%m') = '$ddate' AND item_code='$itemcode' AND branch='$branch'";
			$sqlQueryIR = "SELECT * FROM fds_branch_order WHERE control_no IN ( SELECT control_no FROM fds_order_request WHERE status = 'Closed' ) AND DATE_FORMAT(delivery_date, '%Y-%m') = '$ddate' AND item_code='$itemcode' AND branch='$branch'";
