...
class FDSInventory
{
	public function GetBranchReceivedDataRange($branch,$datefrom,$dateto,$itemcode,$db)
	{
		...
		$QUERY = "SELECT SUM(branch_received) AS QUANTITY FROM fds_branch_order WHERE branch='$branch' AND item_code='$itemcode' AND delivery_date BETWEEN '$datefrom' AND '$dateto' AND branch_received_status='1' AND control_no IN (SELECT control_no FROM fds_order_request WHERE logistics = '1')";
	...
	public function GetTransferOutDataVsbrchreceivingRange($branch,$datefrom,$dateto,$itemcode,$db)
	{
		...
		$QUERY = "SELECT SUM(actual_quantity) AS QUANTITY FROM fds_branch_order WHERE branch='$branch' AND item_code='$itemcode' AND delivery_date BETWEEN '$datefrom' AND '$dateto' AND control_no IN (SELECT control_no FROM fds_order_request WHERE logistics = '1')";
	...
	public function GetInventoryProductionRange($datefrom, $dateto, $itemcode, $db)
    {
        ...
        $query = "SELECT SUM(pcount) AS TOTAL FROM fds_fgts_pcount WHERE trans_date BETWEEN '$datefrom' AND '$dateto' AND item_code='$itemcode'";
        $results = mysqli_query($db, $query);
        if ($results === false) {
            return $query;
        }
	...
	public function GetdatafromtableTransferOutData($column, $transdate, $itemcode, $db)
	{
		...
		$QUERY = "SELECT * FROM fds_branch_order WHERE item_code='$itemcode' AND delivery_date='$transdate' AND control_no IN (SELECT control_no FROM dbc_order_request WHERE logistics = '1')";
	...
	public function Getdatafromtableproduction($column, $transdate, $itemcode, $db)
    {
        ...
        $query = "SELECT * FROM fds_frozendough_production WHERE report_date='$transdate' AND item_code='$itemcode' AND status='Yes'";
        $results = mysqli_query($db, $query);
        if ($results === false) {
            return $query;
        }
	...
	{
		$QUERY = "SELECT * FROM fds_inventory_pcount WHERE trans_date = '$datepcount' AND item_code='$itemcode'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{				
	    ...
	    $query = "SELECT * FROM fds_inventory_pcount WHERE trans_date = '$previousMonthEndDate' AND item_code='$itemcode'";
	    $results = mysqli_query($db, $query);  
	...
	public function GetTransferOutData($itemcode,$transdate,$db)
	{
		...
		$QUERY = "SELECT SUM(actual_quantity) AS QUANTITY FROM fds_branch_order WHERE item_code='$itemcode' AND delivery_date='$transdate' AND control_no IN (SELECT control_no FROM fds_order_request WHERE logistics = '1')";
	...
	public function GetInventoryIn($transdate, $itemcode, $db)
    {
        ...
        $query = "SELECT SUM(pcount) AS TOTAL FROM fds_fgts_pcount WHERE trans_date='$transdate' AND item_code='$itemcode'";
        $results = mysqli_query($db, $query);
        if ($results === false) {
            return $query;
        }
	...
	{
		$query = "SELECT * FROM fds_inventory_stock WHERE item_code='$itemcode' AND stock_before_pcount_date='$trans_date'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
			return 1;
	...
	{
		$query = "SELECT * FROM fds_inventory_pcount WHERE item_code='$itemcode' AND trans_date='$trans_date'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
		    while($ROW = mysqli_fetch_array($results))  
	...
	public function GetMonthlyPcount($cnt_start,$cnt_end,$days_cnt,$itemcode,$month,$year,$db)
	{
		...
		$query = "SELECT * FROM fds_inventory_pcount WHERE trans_date BETWEEN '$startDate' AND '$endDate' AND item_code='$itemcode'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
			$total=0;
	...
	public function getMonthlyIn($week,$itemcode,$month,$year,$db)
	{
		...
		$startDate = $year."-".$month."-".$cnt_start;
		$endDate = $year."-".$month."-".$cnt_end;		
		 $query = "SELECT * FROM fds_receiving receiving
			INNER JOIN fds_receiving_details details
	...
	public function getWeeklyIn($cnt_start,$cnt_end,$days_cnt,$itemcode,$month,$year,$db)
	{
		...
		$query = "SELECT * FROM fds_receiving_details WHERE received_date BETWEEN '$startDate' AND '$endDate' AND item_code='$itemcode'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
			$total=0;
	...
	public function GetInventoryOut($branch,$itemcode,$year,$month,$day,$db)
	{
		...
		$query = "SELECT actual_quantity FROM fds_branch_order WHERE branch='$branch' AND item_code='$itemcode' AND trans_date='$trans_date'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
		    while($ROW = mysqli_fetch_array($results))  
	...
	{
	    $queryDataUpdate = "UPDATE fds_inventory_records SET ending=? WHERE item_code=? AND year=? AND month=?";
	...
	{
	    $queryDataUpdate = "UPDATE fds_inventory_records SET beginning=? WHERE item_code=? AND year=? AND month=?";
	    $stmt = $db->prepare($queryDataUpdate);
	...
	public function getInventoryBeginning($cnt_start,$cnt_end,$days_cnt,$col,$itemcode,$month,$year,$db)
	{
		...
		$query = "SELECT * FROM fds_inventory_pcount WHERE trans_date BETWEEN '$newStartDate' AND '$newEndDate' AND item_code='$itemcode'";
		$results = mysqli_query($db, $query);  
	...
	public function getColumns($tableName,$db)
	{
		...
		if ($stmt)
		{
		    ...
		    while (mysqli_stmt_fetch($stmt)) {
		    	if($tableName == 'fds_inventory_records')
		    	{
	...
	{
		$query = "SELECT * FROM fds_inventory_leadtime";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
		    while($ROW = mysqli_fetch_array($results))  
	...
	{
		$sqlQueryRecords = "SELECT * FROM fds_inventory_records";
