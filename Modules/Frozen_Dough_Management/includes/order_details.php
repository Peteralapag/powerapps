...
<div class="order-wrapper" id="orders" style="position:relative">
	<table style="width: 100%" class="lamesako">
		...
		<tbody>
<?php
	$sqlQuery = "SELECT * FROM fds_branch_order WHERE control_no='$control_no'";
...
?>		
			<tr>
				...
				<td>
					<input id="aqty<?php echo $x; ?>" <?php echo $stock_text; ?> type="<?php echo $type; ?>" class="form-control form-control-sm quantityInput" style="text-align:right" onblur="autoSaveHere('<?php echo $x; ?>','<?php echo $rowid; ?>','<?php echo $quantity; ?>','<?php echo $item_code; ?>','<?php echo $transdate?>')" placeholder="<?php echo $actual_quantity; ?>" value="<?php echo $actual_quantity; ?>">
				</td>
...
<div id="results"></div>
<script>
function autoSaveHere(elemid,rowid,quantity,item_code,transdate)
{
	...
	if(value > onhand){
		value = 0;
		$('#aqty' + elemid).val('0.00');
		swal("Warning", "The actual quantity is much higher than the remaining stock", "warning");
	...
	$.post("./Modules/Frozen_Dough_Management/actions/actions.php", { mode: mode, rowid: rowid, actual_quantity: actual_quantity, control_no: control_no, item_code: item_code, transdate: transdate },
	function(data) {		
		$('#results').html(data);
	});
}
...
function prepRemarks(value,controlno)
{
	...
	$.post("./Modules/Frozen_Dough_Management/actions/actions.php", { mode: mode, control_no: controlno, remarks: value },
	function(data) {		
		$('#results').html(data);
	});
}
