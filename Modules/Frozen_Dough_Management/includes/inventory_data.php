...
{
	$_SESSION['FDS_ITEM_LOCATION'] = $_POST['location'];
} else {
	unset($_SESSION['FDS_ITEM_LOCATION']);
}
if(isset($_POST['limit']) && $_POST['limit'] != '')
{
	$_SESSION['FDS_SHOW_LIMIT'] = $_POST['limit'];
} else {
	unset($_SESSION['FDS_SHOW_LIMIT']);
}
if($_POST['limit'] != '')
{
...
if(isset($_POST['search']))
{
	...
	if(isset($_POST['location']) && $_POST['location'] != '')
	{
		$q = "WHERE fds_itemlist.item_location='$location' AND  (fds_inventory_stock.item_code LIKE '%$search%' OR fds_inventory_stock.item_description LIKE '%$search%')";
	} else {	
		$q = "WHERE fds_inventory_stock.item_code LIKE '%$search%' OR fds_inventory_stock.item_description LIKE '%$search%'";
	}
	...
	if(isset($_POST['location']) && $_POST['location'] != '')
	{
		$search = $_POST['search'];	
		$location = $_POST['location'];
		$q = "WHERE fds_itemlist.item_location='$location'";
	} else {
		$q = "ORDER BY fds_inventory_stock.supplier_id DESC";
	}
}
?>
<style>
...
<?PHP
	$sqlQuery = "
		SELECT * FROM fds_inventory_stock 
        INNER JOIN fds_itemlist ON fds_inventory_stock.item_code = fds_itemlist.item_code
        $q $limit
    ";
    ...
    if ( $results->num_rows > 0 ) 
    {
    	...
    	while($INVROW = mysqli_fetch_array($results))  
		{
			...
			$sqlQueryRecords = "SELECT * FROM fds_inventory_records WHERE year='$year' AND month='$month' AND item_code='$item_code'";
...
function itemDetails(itemcode,item)
{
	...
	$.post("./Modules/Frozen_Dough_Management/apps/inventory_item_details.php", { item_code: itemcode, item: item },
	function(data) {		
