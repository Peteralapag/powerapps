...
<table style="width: 100%" class="table table-bordered table-striped table-hover">
	...
	<tbody>	
<?PHP
	$sqlQuery = "SELECT * FROM fds_frozendough_production WHERE report_date='$transdate' ORDER BY receiving_detail_id DESC";
    ...
    if ( $results->num_rows > 0 ) 
    {
    	...
    	while($RECVROW = mysqli_fetch_array($results))  
		{
			...
			$pcount = $status == 'No'? '' : $function->GetDataByRecevingId('pcount','fds_fgts_pcount',$id,$db);
			$remarks = $status == 'No'? '' : $function->GetDataByRecevingId('remarks','fds_fgts_pcount',$id,$db);
...
?>
	...
	<table style="width: 100%" class="table table-bordered table-striped">
		...
		$sqlQuery = "SELECT DISTINCT item_description,item_code,created_time,batch_received,quantity_received,charge,over_yield,confirmed_by,receiving_detail_id FROM fds_frozendough_production WHERE report_date='$transdate' AND status='Yes' ORDER BY receiving_detail_id DESC";
		$results = mysqli_query($db, $sqlQuery); 
	    ...
	    if ( $results->num_rows > 0 ) 
	    {
	    	...
	    	while($RECVROW = mysqli_fetch_array($results))  
			{
				...
				$pcount = $function->GetDataByRecevingId('pcount','fds_fgts_pcount',$receivingdetailid,$db);
				$remarks = $function->GetDataByRecevingId('remarks','fds_fgts_pcount',$receivingdetailid,$db);
...
{
	var mode = 'voidfrozendough';
	rms_reloaderOn('Loading...');
	$.post("./Modules/Frozen_Dough_Management/actions/actions.php", { mode: mode, id: id, itemcode: itemcode, category: category, itemdescription: itemdescription, qty: qty, reportdate: reportdate, shift: shift },
	function(data) {		
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
function receiveThis(n,id,itemcode,category,itemdescription,qty,reportdate,shift)
{
	...
	rms_reloaderOn('Loading...');
	$.post("./Modules/Frozen_Dough_Management/actions/actions.php", { mode: mode, id: id, itemcode: itemcode, category: category, itemdescription: itemdescription, qty: qty, reportdate: reportdate, shift: shift, pcount: pcount, remarks: remarks },
	function(data) {		
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});		
