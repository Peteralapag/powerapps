...
<div class="smnav-header">
	...
	<span class="date-shell">
		...
		<input type="hidden" id="recipient" style="width:490px;" class="form-control form-control-sm" value="FROZEN DOUGH" disabled>
		...
		<!--span>
			<button type="button" class="btn btn-success btn-sm" style="margin-left:15px" onclick="filterviadate()"><i class="fa fa-filter" aria-hidden="true"></i>&nbsp;Filter date</button>
		</span-->
		
		<span style="margin-left:30px; margin-top:6px; font-size:12px">
			To view all orders organized by items with date, please <span style="color:red; cursor:pointer" onclick="Check_Permissions('p_view',openMenuGranted,'fd_branch_order_receiving','Branch Order Receiving')">click</span> here.
		</span>
...
function filterviadate()
{
	...
	$.post("./Modules/Frozen_Dough_Management/apps/date_filtering_form.php", { limit: limit, recipient: recipient, ord: ord },
	function(data) {		
...
function getRecipientData()
{
	...
	$.post("./Modules/Frozen_Dough_Management/includes/branch_order_receiving_data.php", { limit: limit, recipient: recipient, ord: ord },
	function(data) {		
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
$(function()
{
	...
	$('#search').keyup(function()
	{
		...
		$.post("./Modules/Frozen_Dough_Management/includes/branch_order_receiving_data.php", { limit: limit, search: search, recipient: recipient, ord: ord },
		function(data) {
			$('#smnavdata').html(data);
		});
