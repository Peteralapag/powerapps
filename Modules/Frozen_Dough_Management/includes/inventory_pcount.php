...
if(isset($_SESSION['FDS_ITEM_LOCATION']))
{
	$location = $_SESSION['FDS_ITEM_LOCATION'];
} else {
	$location = "";
}
if(isset($_SESSION['FDS_TRANSDATE']))
{
...
<div class="subnav-header">
	...
	<select id="location" class="form-control form-control-sm" style="width:200px;margin-right:10px" disabled>
		<?PHP echo $function->GetItemLocation('FROZEN DOUGH',$db); ?>
	</select>
...
function load_pcount()
{
	...
	$.post("./Modules/Frozen_Dough_Management/includes/phycount_data.php", { location: location, trans_date: trans_date, limit: limit },
	function(data) {		
...
$(document).ready(function()
{
	$('#search').keyup(function()
	{
		...
		var location = $('#location').val();
		var trans_date = $("#transdate").val();
		$.post("./Modules/Frozen_Dough_Management/includes/phycount_data.php", { search: search, location: location, trans_date: trans_date, limit: limit },
		function(data) {		
			$('#phycountdata').html(data);
			rms_reloaderOff();
		});	
