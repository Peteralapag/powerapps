...
$trans_date = $_SESSION['FDS_TRANSDATE'];
?>
<style>
...
<div class="smnav-header">
	...
	<div class="date-shell">
		<input id="transdate" type="date" class="form-control form-control-sm" style="width:150px" value="<?php echo $trans_date; ?>" onchange="searchMe()">
	</div>
	...
	<span style="margin-left:30px; margin-top:6px; font-size:12px">
		Pending approval <span class="badge bg-danger"><?php echo $function->getPendingApprovalCount($db)?></span><!--, please <span style="color:red; cursor:pointer" onclick="Check_Permissions('p_view',openMenuGranted,'frozendough_receiving','Frozen Dough Receiving')">click</span> here.-->
	</span>
...
function viewSummary(){
	...
	$.post("./Modules/Frozen_Dough_Management/includes/frozendough_summary.php", { },
	function(data) {		
	...
	$.post("./Modules/Frozen_Dough_Management/includes/frozendough_summary_data.php", { transdate: transdate },
	function(data) {
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
function searchMe(){
	...
	rms_reloaderOn('Loading...');
	$.post("./Modules/Frozen_Dough_Management/includes/frozendough_receiving_data.php", { transdate: transdate },
	function(data) {
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
function getRecipientData()
{
	...
	var ord = $('#ord').val();
	rms_reloaderOn('Loading...');	
	$.post("./Modules/Frozen_Dough_Management/includes/frozendough_receiving_data.php", { limit: limit, recipient: recipient, ord: ord },
	function(data) {		
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
$(function()
{
	...
	$('#search').keyup(function()
	{
		...
		$.post("./Modules/Frozen_Dough_Management/includes/frozendough_receiving_data.php", { limit: limit, search: search, recipient: recipient, ord: ord },
		function(data) {
			$('#smnavdata').html(data);
		});
