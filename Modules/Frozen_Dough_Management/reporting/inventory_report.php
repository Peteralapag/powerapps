...
$_SESSION['FDS_ITEMCATEGORY'] = $_POST['category'];
if(isset($_SESSION['FDS_CLUSTER']))
{
	$cluster = $_SESSION['FDS_CLUSTER'];
	if(isset($_SESSION['FDS_BRANCH']))
	{
		$thebranch = $_SESSION['FDS_BRANCH'];
	} else {
		$thebranch = '';
	}
} else {
...
{
	$theyears = $_SESSION['FDS_YEARS'];
} else {
	$theyears = date("Y");	
}
if(isset($_SESSION['FDS_MONTHS']))
{
	$themonths = $_SESSION['FDS_MONTHS'];
} else {
	$themonths = date("m");	
}
if(isset($_SESSION['FDS_DAYS']))
{
	$thedays = $_SESSION['FDS_DAYS'];
} else {
	$thedays = date("d");	
}
...
function setBranch(branch)
{
	...
	var mode = 'setbranch';
	$.post("./Modules/Frozen_Dough_Management/actions/actions.php", { mode: mode, cluster: cluster, branch: branch },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
...
{
	var mode = 'setcluster';
	$.post("./Modules/Frozen_Dough_Management/actions/actions.php", { mode: mode, cluster: cluster },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
		loadBranch(cluster);
...
function loadBranch(cluster)
{
	...
	$.post("./Modules/Frozen_Dough_Management/actions/actions.php", { mode: mode, branch: branch, cluster: cluster },
	function(data) {		
		$('#branches').html(data);
		rms_reloaderOff();
	}); 
...
function GenerateExcelDaily()
{
		...
		$("#genxcel_page").attr("src", "./Modules/Frozen_Dough_Management/reporting/generate_excel_daily.php?years=" + theyears + "&months=" + themonths + "&days=" + thedays);
		$("#genxcel").show();
...
function inventoryPcount()
{
	...
	$.post("./Modules/Frozen_Dough_Management/reporting/inventory_setup.php", { theyears: theyears, themonths: themonths, thedays: thedays, location: location },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
...
function loadDataNow()
{
	...
	$.post("./Modules/Frozen_Dough_Management/reporting/inventory_sheet_data.php", { theyears: theyears, themonths: themonths, thedays: thedays },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
