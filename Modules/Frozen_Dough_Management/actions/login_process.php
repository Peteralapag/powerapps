...
if($mode=='c10cc6b684e1417e8ffa924de1e58373')
{
	...
	$pass = $_POST['password'];
	$user_app = "Frozen Dough Management";	
    ...
    { 
		...
		if($void == 0)
		{
			echo checkFDSManager($idcode,$db);
			if(checkFDSManager($idcode,$db) != '')		
			{
				$_SESSION['fds_user_recipient'] = checkFDSManager($idcode,$db);
			}
			$_SESSION['fds_username'] = $username;
			$_SESSION['fds_company'] = $company;
			$_SESSION['fds_appnameuser'] = $employee;
			$_SESSION['fds_userlevel'] = $user_level;
			$_SESSION['fds_userrole'] = $user_role;
			$_SESSION['fds_application'] = $user_app;
			$_SESSION['FDS_TRANSDATE'] = $formattedDate;	
			$cmd = '';
			$cmd .='				
				<script>
					$("#formmodal").hide();
...
function checkFDSManager($idcode,$db)
{
	$checkManager = "SELECT * FROM fds_user_recipient WHERE idcode='$idcode'";
