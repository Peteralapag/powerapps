...
{
	$year = $_SESSION['BINALOT_YEAR'];
} else {
	$year = date("Y");
}
if(isset($_SESSION['BINALOT_MONTH']))
{
	$month = $_SESSION['BINALOT_MONTH'];
} else {
	$month = date("m");
}
if(isset($_SESSION['BINALOT_WEEK']))
{
	$week = $_SESSION['BINALOT_WEEK'];
} else {
	$week = 1;
}
?>
...
function loadReport()
{
	...
	setTimeout(function()
	{
		$.post("./Modules/Binalot_Management/binalot_report/" + page, { recipient: recipient, branch: branch, year: year, month: month },
		function(data) {		
			$('#iwd').html(data);
			rms_reloaderOff();
		});
