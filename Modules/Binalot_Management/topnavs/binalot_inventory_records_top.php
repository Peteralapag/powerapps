...
$_SESSION['BINALOT_TABLE'] = $tableName;
...
if(isset($_SESSION['BINALOT_YEAR']))
{
	$inv_year = $_SESSION['BINALOT_YEAR'];
} else {
	$inv_year = date("Y");
}
...
<div class="search-shell" style="margin-right:10px;width:180px;position:relative">
	<select id="subinvstatus" style="width:180px" class="form-control form-control-sm" onchange="loadSubInvStats(this.value)">
		<?php echo $function->getBINALOTSubReport($tableName,$inv_submenu,$db); ?>
	</select>
</div>
...
function GenerateExcel(table)
{
	...
	if(sessionStorage.stringKo === undefined || sessionStorage.stringKo === null)
	{
		...
		{
			console.log(table);
			$("#genxcel_page").attr("src", "./Modules/Binalot_Management/reporting/generate_excel_inventory_status.php?table=" + table + "&columns=" + resultString + "&month=" + month + "&year=" + year + "&limit=" + limit + "&category=" + category);
		}
		else if($('#subinvstatus').val() == 'inventory_monthly')
		{
			$("#genxcel_page").attr("src", "./Modules/Binalot_Management/reporting/generate_excel_inventory_monthly.php?table=" + table + "&columns=" + resultString + "&month=" + month + "&year=" + year + "&limit=" + limit + "&category=" + category);
		}
		$("#genxcel").show();
