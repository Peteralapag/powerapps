...
<div class="process-header">
	...
	<span style="margin-left: auto">
		<button class="btn btn-warning btn-sm" onclick="checkBINALOTRequest()"><i class="fa-solid fa-bells color-white"></i>&nbsp;&nbsp;Pending Request</button>
...
<div style="padding:10px; display: <?php echo $statusss?>">
	...
	<?php if($function->GetOrderStatus($control_no,'status',$db) == 'Void') { ?>
		<img src="./Modules/Binalot_Management/images/void.png" alt="Void Status" style="width: 80px; height: 80px; margin-left: 20px;">
	<?php } else { ?>
...
function checkBINALOTRequest()
{	
	var recipient = $('#recipient').val();
	$('#modaltitle').html("RE-OPEN ORDER REQUEST");	
	$.post("./Modules/Binalot_Management/includes/pending_request_data.php", { recipient: recipient, recipient: recipient },
	function(data) {		
...
function loadPickListSingle()
{
	...
	$.post("./Modules/Binalot_Management/includes/pick_list_loader.php", { mode: mode, transdate: transdate, branch: branch, control_no: control_no  },
	function(data) {		
...
function loadPickList(controlno,transdate)
{
	$('#modaltitlePDF').html("Print Pick Lists");
	var transdate = '<?php echo $trans_date; ?>';
	$.post("./Modules/Binalot_Management/includes/pick_list_loader.php", { transdate: transdate, control_no: controlno },
	function(data) {		
		$('#pdfviewer_page').html(data);
		$('#pdfviewer').show();
	});
...
$(function()
{
	if('<?php echo $showdtls; ?>' == 1)
	{
		...
		{
			var control_no = '<?php echo $control_no; ?>';
			$.post("./Modules/Binalot_Management/includes/order_details.php", { control_no: control_no },
			function(data) {		
	...
	if('<?php echo $function->GetOrderStatus($control_no,"order_received",$db) == 0?>')
	{
		var control_no = '<?php echo $control_no; ?>';
		var branch = '<?php echo $branch; ?>';
		$.post("./Modules/Binalot_Management/includes/view_this_controlno.php", { controlno: control_no, branch: branch },
		function(data) {		
			$('#orderdetails').html(data);
		});
	}
...
{
	$.post("./Modules/Binalot_Management/includes/delivery_details.php", { control_no: control_no },
	function(data) {		
		$('#drdetails').html(data);
	});
}
...
function savePickList(controlno)
{
	...
	setTimeout(function()
	{
		$.post("./Modules/Binalot_Management/actions/picklist_process.php", { control_no: controlno },
		function(data) {		
			$('#results').html(data);
			rms_reloaderOff();
