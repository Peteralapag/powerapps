...
<div class="smnav-header">
	<div class="search-shel">
		<select id="fdsmodule" class="form-control form-control-sm" style="width:200PX">
			<?php echo $function->getBINALOTReport($binalot_table,$db); ?>
		</select>
	</div>
	<span class="date-shell" id="dateinputs">
		<select class="form-control form-control-sm" style="margin-left:5px;width:150px"><?php echo $function->getBINALOTSubReport($binalot_table,$db); ?></select>
	...
	<div class="search-shell">
		<div class="btn-group" role="group" aria-label="Ronan Sarbon">
			...
			<button class="btn btn-success btn-sm" style="width:100px" onclick="GenerateExcel('<?php echo $binalot_table; ?>')">EXCEL&nbsp;&nbsp;&nbsp;<i class="fa-solid fa-file-excel"></i></button>	
		</div>
	</div>
...
function GenerateExcel(table)
{
	if(sessionStorage.stringKo === undefined || sessionStorage.stringKo === null)
	{
		...
		$("#genxcel_page").attr("src", "./Modules/Binalot_Management/reporting/generate_excel.php?table=" + table + "&columns=" + sessionStorage.stringKo);
		$("#genxcel").show();
...
$(function()
{
	if('<?php echo $binalot_table; ?>' == 'binalot_inventory_records')
	{
		$('#dateinputs').show();
	} else {
		$('#dateinputs').hide();
	...
	$('#date_from,#date_to').on('change', function(event)
	{
		...
		loadWithDate('<?php echo $binalot_table; ?>',datefrom,dateto);
	});
});
function loadWithDate(table,datefrom,dateto)
{
	var limit = $('#limit').val();
	$.post("./Modules/Binalot_Management/reporting/inventory_report.php", { table: table, datefrom: datefrom, dateto: dateto },
	function(data) {		
		$('#smnavdata').html(data);
	});
}
function loadModule(table)
{
	var limit = $('#limit').val();
	 ...
	 {
		$.post("./Modules/Binalot_Management/reporting/show_report.php", { table: table },
		function(data) {		
			$('#smnavdata').html(data);
		});	 	
	 } else {
 	 	var datefrom = $('#date_from').val();
	 	var dateto = $('#date_to').val();
		$.post("./Modules/Binalot_Management/reporting/inventory_report.php", { limit: limit },
		function(data) {		
			$('#smnavdata').html(data);
		});
	}
