<?php
class BINALOTFunctions
{

	public function GetBranchOrderRecevingServeAllPending($transdate,$db)
	{
		$QUERY = "SELECT * FROM binalot_order_request WHERE trans_date = '$transdate' AND status='Submitted'";
	...
	{
	    $query = "SELECT item_description, report_date, posted_by FROM binalot_binalot_production WHERE status = 'No' ORDER BY report_date DESC";
	...
	public function GetTransferOutDataViaBranchReceived($branch,$itemcode,$transdate,$db)
	{
	    ...
	    $QUERY = "SELECT SUM(actual_quantity) AS QUANTITY 
	              FROM binalot_branch_order 
	              WHERE item_code = ? 
	              AND trans_date = ? 
	              AND branch = ? 
	              AND control_no IN (
	                  SELECT control_no 
	                  FROM binalot_order_request 
	                  WHERE status = 'Closed' 
	...
	function countDetectEmployeeChargedBAKERONLY2($module, $chargeno, $db) {
	    ...
	    $query = "SELECT SUM($module) as total FROM binalot_charges_extension_record WHERE chargecodeno = ? AND employee_type = 'BAKER'";
	...
	function countDetectEmployeeChargedNONEBAKERONLY($module, $chargeno, $db) {
	    ...
	    $query = "SELECT SUM($module) as total FROM binalot_charges_extension_record WHERE chargecodeno = ? AND employee_type = 'NONE BAKER'";
	...
	function updateChargesByChargedBAKERONLY($chargeno, $new_quantity, $new_total, $db) {
	    ...
	    $stmt = $db->prepare("UPDATE binalot_charges_extension_record SET quantity = ?, total = ? WHERE chargecodeno = ? AND employee_type = 'BAKER'");
	...
	function countDetectEmployeeChargedBAKERONLY($chargeno, $db) {
	    ...
	    $stmt = $db->prepare("SELECT COUNT(*) as record_count FROM binalot_charges_extension_record WHERE chargecodeno = ? AND employee_type = 'BAKER'");
    	$stmt->bind_param("s", $chargeno);	
	...
	function updateChargesByChargeCode($chargeno, $new_quantity, $new_total, $db) {
	    ...
	    $stmt = $db->prepare("UPDATE binalot_charges_extension_record SET quantity = ?, total = ? WHERE chargecodeno = ?");
	    $stmt->bind_param("dds", $new_quantity, $new_total, $chargeno);
...
//	        return "Error updating record: " . $db->error;
	...
	function countDetectEmployeeCharged($chargeno, $db) {
	    ...
	    $stmt = $db->prepare("SELECT COUNT(*) as record_count FROM binalot_charges_extension_record WHERE chargecodeno = ?");
	...
	public function getValueSummaryviadate($datefrom, $dateto, $itemcode, $db)
	{
	    $QUERY = "
	        ...
	        FROM binalot_branch_order b
	        JOIN binalot_order_request o ON b.control_no = o.control_no
	...
	{
		$sqlQueryStk = "SELECT * FROM binalot_charges_extension_record WHERE chargecodeno = '$chargecodeno'";
	...
	{
		$returnquantity = $this->Getfromthetable('quantity','binalot_retouch',$rowid,$itemcode,$db);
		...
		$queryDataUpdate = "UPDATE binalot_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand-'$returnquantity', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
	...
	public function getIntoDbcTblitemslist($datefrom, $dateto, $db)
	{
	    ...
	    $branches = [];
	    $branchQuery = "SELECT * FROM binalot_tbl_branch WHERE status = 1";  
	    ...
	    $query = "SELECT * FROM binalot_itemlist WHERE active = 1 ORDER BY ordered ASC";  
	    $result = mysqli_query($db, $query);  
	    $i = 1;
	...
	{
		$query ="SELECT * FROM binalot_tbl_branch WHERE status = 1";  
		$result = mysqli_query($db, $query);  
		while($ROWS = mysqli_fetch_array($result))  
		{
	...
	{
		$QUERY = "SELECT * FROM binalot_tbl_branch WHERE branch_id='$branchid'";
		$RESULTS = mysqli_query($db, $QUERY);
	...
	public function insertIntoDbcTblBranchTable($branch, $branchid, $active, $db)
	{
	    ...
	    $checkQuery = "SELECT * FROM binalot_tbl_branch WHERE branch_id = ?";
	    ...
	    if ($result->num_rows > 0) {
	        ...
	        $deleteQuery = "DELETE FROM binalot_tbl_branch WHERE branch_id = ?";
	        ...
	        // Insert new branch
	        $insertQuery = "INSERT INTO binalot_tbl_branch (branch_id, branch, status) VALUES (?, ?, ?)";
	...
	{
		$quantity = $this->Getfromthetable('quantity','binalot_'.$table,$rowid,$itemcode,$db);
		
		$stockbeforepcountdate = $this->GetInventoryStockStocknHandReturn('stock_before_pcount_date',$itemcode,$db);
		$stockbeforepcount = $this->GetInventoryStockStocknHandReturn('stock_before_pcount',$itemcode,$db);
		
		$queryDataUpdate = "UPDATE binalot_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand+'$quantity', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	public function otherinventoryoutdata($itemcode,$itemval,$dateupdated,$updatedby,$db)
	{
		...
		$queryDataUpdate = "UPDATE binalot_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand-'$itemval', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	{
		$returnquantity = $this->Getfromthetable('quantity','binalot_return',$rowid,$itemcode,$db);
	...
	public function insertReturnData($itemcode,$returnval,$dateupdated,$updatedby,$db)
	{
		...
		$queryDataUpdate = "UPDATE binalot_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand+'$returnval', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	{
		$sqlQueryStk = "SELECT $module FROM binalot_inventory_stock WHERE item_code='$itemcode'";
	    $stkResults = mysqli_query($db, $sqlQueryStk);
	    $val = 0;
	    if ($stkResults->num_rows > 0)
	    {
	...
	{
		$QUERY = "SELECT * FROM binalot_datelock_checker WHERE report_date='$transdate'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	{
		$QUERY = "SELECT * FROM binalot_datelock_checker WHERE report_date='$transdate' AND status='1'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	public function executeInventory($itemcode,$pcount,$transdate,$dateupdated,$updatedby,$db)
	{
		...
		$queryDataUpdate = "UPDATE binalot_inventory_stock SET stock_before_pcount_date='$transdate',stock_before_pcount='$valuebeforepcount', stock_in_hand='$pcount', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	{
		$sqlQueryStk = "SELECT stock_in_hand FROM binalot_inventory_stock WHERE item_code='$itemcode'";
	    $stkResults = mysqli_query($db, $sqlQueryStk);
	    $val = 0;
	    if ($stkResults->num_rows > 0)
	    {
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM binalot_binalot_production WHERE item_code = '$itemcode' AND MONTH(report_date) = MONTH('$transdate')";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM binalot_inventory_pcount WHERE trans_date='$transdate' AND item_code='$itemcode'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
		$QUERY = "SELECT * FROM binalot_inventory_pcount WHERE trans_date = '$transdate'";
		$RESULTS = mysqli_query($db, $QUERY);
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	{
		$query ="SELECT * FROM binalot_itemlist WHERE item_code='$itemcode'";  
		$RESULTS = mysqli_query($db, $query);
		$val = '';
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	public function getPcountExist($transdate, $db) {
	    ...
	    $query = "SELECT trans_date FROM binalot_inventory_pcount WHERE trans_date='$transdate'";
	...
	{
		$QUERY = "SELECT * FROM binalot_binalot_production WHERE item_code='$itemcode' AND report_date='$transdate' AND status <> 'Void'";
		$RESULTS = mysqli_query($db, $QUERY);
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	{
		$QUERY = "SELECT * FROM binalot_binalot_production WHERE report_date='$transdate' AND status='Yes'";
		$RESULTS = mysqli_query($db, $QUERY);
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	public function GetTransferOutData($itemcode,$transdatefrom,$transdateto,$db)
	{
		...
		$QUERY = "SELECT SUM(actual_quantity) AS QUANTITY FROM binalot_branch_order WHERE item_code='$itemcode' AND trans_date BETWEEN '$transdatefrom' AND '$transdateto' AND control_no IN (SELECT control_no FROM binalot_order_request WHERE status = 'Closed' AND order_delivered = 1)";
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM binalot_binalot_production WHERE report_date BETWEEN '$datefrom' AND '$dateto' AND item_code='$itemcode' AND status='Yes'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM binalot_fgts_pcount WHERE trans_date BETWEEN '$datefrom' AND '$dateto' AND item_code='$itemcode'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
	    $query ="SELECT COUNT(*) AS pendingTotal FROM binalot_binalot_production WHERE status='No'";  
	...
	{
		$query ="SELECT * FROM binalot_itemlist";  
		$result = mysqli_query($db, $query);  
	...
	{
		$queryDataUpdate = "UPDATE binalot_branch_order SET delivery_date='$delivery_date' WHERE control_no='$control_no'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			return $db->error;
	...
	{
		$QUERY = "SELECT * FROM binalot_classifications_records WHERE item_code='$item_code' AND flag=0";
		$RESULTS = mysqli_query($db, $QUERY);		
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
	...
	{
		$QUERY = "SELECT * FROM binalot_order_request WHERE item_code='$item_code'";
		$RESULTS = mysqli_query($db, $QUERY);		
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
	...
	{
		$queryDataUpdate = "UPDATE binalot_order_request SET status='Submitted', logistics=0, order_transit=0 WHERE control_no='$control_no'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
			//echo "SUCCESS";
		} else {
	...
	{
		$QUERY = "SELECT * FROM binalot_inventory_stock WHERE item_code='$item_code'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
			...
			/* ------------------------------------------------------------------------------------------- */
				$queryDataUpdate = "UPDATE binalot_inventory_stock SET stock_in_hand='$new_stock' WHERE item_code='$item_code'";
				if ($db->query($queryDataUpdate) === TRUE)
				{
				/* ##################################################################### */
					$queryDataUpdate = "UPDATE binalot_order_request SET status='Submitted', logistics=0, order_transit=0 WHERE control_no='$control_no'";
	...
	{
		$queryBO = "SELECT * FROM binalot_branch_order WHERE control_no='$control_no'";
		...
		$queryPL = "SELECT * FROM binalot_picklist WHERE control_no='$control_no'";
	...
	{
		$QUERY = "SELECT * FROM binalot_receiving WHERE receiving_id='$receiving_id'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
	...
	{
		$query = "SELECT * FROM binalot_receiving WHERE receiving_id='$receiving_id' AND status='Closed'";
		$result = mysqli_query($db, $query);		
	    if ( $result->num_rows > 0 ) 
	    {
	    	return 1;
	...
	{
		$queryBO = "SELECT * FROM binalot_branch_order WHERE control_no='$control_no' AND branch='$branch'";
		$resultBO = mysqli_query($db, $queryBO);		
		if ($resultBO)
		{
		    $boCount = mysqli_num_rows($resultBO);
	...
	{
		$QUERY = "SELECT * FROM binalot_inventory_stock WHERE item_code='$itemcode'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
	...
	public function GetBINALOTReports($report,$db)
	{
		$query = "SELECT * FROM binalot_reporting_option WHERE active=1 ORDER BY ordering ASC";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	public function DoAuditLogs($log_date,$activity,$log_by,$db)
	{
		...
		$queryInsert = "INSERT INTO binalot_audit_logs ($column) VALUES ($insert)";
	...
	{
		$query = "SELECT * FROM binalot_item_location WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	{
		$query = "SELECT * FROM binalot_inventory_pcount WHERE item_code='$itemcode' AND trans_date='$transdate'";
		$results = $db->query($query);			
	    if($results->num_rows > 0)
	    {
		    while($ROW = mysqli_fetch_array($results))  
	...
	{
		$query = "SELECT * FROM binalot_inventory_records WHERE item_code='$itemcode' AND month='$month' AND year='$year'";
		$results = $db->query($query);			
	    if($results->num_rows > 0)
	    {
		    while($ROW = mysqli_fetch_array($results))  
	...
	public function getBINALOTSubReport($table,$submenu,$db)
	{
		$query = "SELECT * FROM binalot_reporting_option_sub WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
			$return = '';
	...
	public function getBINALOTReport($table,$db)
	{
		$query = "SELECT * FROM binalot_reporting_option WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	{
		$query = "SELECT * FROM binalot_form_numbering WHERE id=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
		    while($ROW = mysqli_fetch_array($results))  
	...
	{
		$QUERY = "SELECT * FROM binalot_branch_order_remarks WHERE control_no='$mrs_no'";
//		return $QUERY;
//		exit();
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
	...
	{
		$query = "SELECT * FROM binalot_recipient WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	{
		$query = "SELECT * FROM binalot_supplier WHERE id='$supplier_id'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
		    while($ROW = mysqli_fetch_array($results))  
	...
	{
		$query = "SELECT * FROM binalot_supplier";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	{
		$query = "SELECT * FROM binalot_units_measures";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	{
		$query = "SELECT * FROM binalot_item_category WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
