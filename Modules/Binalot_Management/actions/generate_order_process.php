...
if($mode == 'newrequest')
{
	...
	$queryCN = "SELECT * FROM binalot_form_numbering WHERE mrs_number='$controlno'";
    ...
    if ( $checkRes->num_rows === 0 ) 
    {
		...
		$queryInsert = "INSERT INTO binalot_order_request ($column)";
		$queryInsert .= "VALUES($insert)";
		if ($db->query($queryInsert) === TRUE)
		{
			$queryDataUpdate = "UPDATE binalot_form_numbering SET mrs_number='$new_control_no' WHERE id=1";
...
if($mode == 'updaterequest')
{
	...
	$queryDataUpdate = "UPDATE binalot_order_request SET $update WHERE request_id='$request_id'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
		print_r('
			<script>
