...
if (mysqli_num_rows($result) > 0) {
    ...
    {			    
       ...
       $branch = $row['branch'];
		...
		$queryChk = "SELECT * FROM binalot_picklist WHERE control_no='$control_no' AND item_code='$item_code'";
...
function saveToPickList($data,$function,$date_user,$app_user,$db)
{
	for($i = 0; $i < count($data); $i++)
	{
		...
		$queryInsert = "INSERT INTO binalot_picklist ($column) VALUES ($insert)";
