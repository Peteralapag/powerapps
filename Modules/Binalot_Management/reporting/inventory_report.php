...
$_SESSION['BINALOT_INVSUBMENU'] = $_POST['invpage'];
$_SESSION['BINALOT_ITEMCATEGORY'] = $_POST['category'];
$_SESSION['BINALOT_ITEMCATEGORY'] = $_POST['category'];
...
if(isset($_SESSION['BINALOT_YEARS']))
{
	$theyears = $_SESSION['BINALOT_YEARS'];
} else {
	$theyears = date("Y");	
}
if(isset($_SESSION['BINALOT_MONTHS']))
{
	$themonths = $_SESSION['BINALOT_MONTHS'];
} else {
	$themonths = date("m");	
}
if(isset($_SESSION['BINALOT_DAYS']))
{
	$thedays = $_SESSION['BINALOT_DAYS'];
} else {
	$thedays = date("d");	
}
...
function setBranch(branch)
{
	...
	var mode = 'setbranch';
	$.post("./Modules/Binalot_Management/actions/actions.php", { mode: mode, cluster: cluster, branch: branch },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
...
{
	var mode = 'setcluster';
	$.post("./Modules/Binalot_Management/actions/actions.php", { mode: mode, cluster: cluster },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
		loadBranch(cluster);
...
function loadBranch(cluster)
{
	...
	$.post("./Modules/Binalot_Management/actions/actions.php", { mode: mode, branch: branch, cluster: cluster },
	function(data) {		
		$('#branches').html(data);
		rms_reloaderOff();
	}); 
...
function GenerateExcelDaily()
{
		...
		$("#genxcel_page").attr("src", "./Modules/Binalot_Management/reporting/generate_excel_daily.php?years=" + theyears + "&months=" + themonths + "&days=" + thedays);
		$("#genxcel").show();
...
function inventoryPcount()
{
	...
	$.post("./Modules/Binalot_Management/reporting/inventory_setup.php", { theyears: theyears, themonths: themonths, thedays: thedays, location: location },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
...
function loadDataNow()
{
	...
	$.post("./Modules/Binalot_Management/reporting/inventory_sheet_data.php", { theyears: theyears, themonths: themonths, thedays: thedays },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
