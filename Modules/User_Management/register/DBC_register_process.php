...
if($mode=='register')
{
	...
	if(CheckIdcodeExist($idcode,$db) == '1') {
		...
		echo '<script>app_alert("Warning","IDCODE already DBC registered","warning","Ok","","no");</script>';
		exit();
	}
...
{
	$query = "SELECT * FROM tbl_system_permission WHERE idcode='$idcode' AND applications='DBC Ordering System' AND modules='Main'";
...
function branchOrderingMain($idcode,$acctname,$username,$db){
	...
	$sqlInsert .= "VALUES('$idcode','$acctname','$username','10','DBC Branch Ordering System','Main','1')";
...
function branchOrderingCreateOrder($idcode,$acctname,$username,$db){
	...
	$sqlInsert .= "VALUES('$idcode','$acctname','$username','10','DBC Branch Ordering System','Create Order','1','1','1','1','1','1','1')";
	$result=$db->query($sqlInsert);
	if ($result === TRUE){}else{}
}
function branchOrderingCreateRequest($idcode,$acctname,$username,$db){
	$sqlInsert = "INSERT INTO tbl_system_permission (`idcode`,`acctname`,`username`,`userlevel`,`applications`,`modules`,`p_view`,`p_read`,`p_add`,`p_write`,`p_edit`,`p_delete`,`p_update`)";
	$sqlInsert .= "VALUES('$idcode','$acctname','$username','10','DBC Branch Ordering System','Create Request','1','1','1','1','1','1','1')";
	$result=$db->query($sqlInsert);
	if ($result === TRUE){}else{}
}
function branchOrderingOrderTracking($idcode,$acctname,$username,$db){
	$sqlInsert = "INSERT INTO tbl_system_permission (`idcode`,`acctname`,`username`,`userlevel`,`applications`,`modules`,`p_view`)";
	$sqlInsert .= "VALUES('$idcode','$acctname','$username','10','DBC Branch Ordering System','Order Tracking','1')";
	$result=$db->query($sqlInsert);
	if ($result === TRUE){}else{}
}
function branchOrderingMyOrder($idcode,$acctname,$username,$db){
	$sqlInsert = "INSERT INTO tbl_system_permission (`idcode`,`acctname`,`username`,`userlevel`,`applications`,`modules`,`p_view`)";
	$sqlInsert .= "VALUES('$idcode','$acctname','$username','10','DBC Branch Ordering System','My Orders','1')";
	$result=$db->query($sqlInsert);
	if ($result === TRUE){}else{}
}
