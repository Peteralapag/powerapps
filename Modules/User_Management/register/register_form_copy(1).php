...
<table class="table" style="width:420px">
	<tr>
		<th>IDCODE:</th>
		<td><input type="text" id="idcode" class="form-control" onchange="idcodeSearch()" autocomplete="off" placeholder="Your IDCode"></td>
	</tr>
	<tr>
		<th>FIRST NAME:</th>
	...
	<tr>
		<th>USERNAME:</th>
		<td><input type="text" id="username" class="form-control" autocomplete="off"></td>
	</tr>
	<tr>
		<th>PASSWORD:</th>
	...
	<tr>
		...
		<td class="tsa-a-na-may-tae">
			<input id="captcha" style="text-align:center" type="text" class="form-control" autocomplete="off" placeholder="ENTER CODE">
	...
	<tr>
		<td colspan="2" style="text-align:center">
			<button id="pedtroalagay" type="button" class="btn btn-primary btn-lg w-50"><i class="fa-solid fa-user-plus"></i>&nbsp;&nbsp;Register</button>
...
$(function()
{	
	$('#pedtroalagay').click(function()
	{
		register();
	});
	var mode = 'captchaGenerate';
	$.post("../Modules/User_Management/register/register_process.php", { mode:mode },
	function(data) {
...
function idcodeSearch(){
	var idcode = $("#idcode").val();
	var mode = 'idcodeSearch';
	
	$.post("../Modules/User_Management/register/register_process.php", { mode:mode,idcode:idcode },
	function(data) {
		$('#results').html(data);
	});
}
function register()
{
	...
	else
	{
		if(password!=confirmpassword){
			app_alert("Warning","Password not match","warning","Ok","","no");
			return false;
		}
		if(captcha!=captchaconfirm){
			app_alert("Warning","Verification Code incorrect","warning","Ok","","no");
			return false;
		}		
		rms_reloaderOn("Registering user...");
		$('#pedtroalagay').attr('disabled', true);
		setTimeout(function()
		{
			$.post("../Modules/User_Management/register/register_process.php", { mode:mode,idcode:idcode,firstname:firstname,lastname:lastname,username:username,password:password,branch:branch,cluster:cluster },
			function(data) {
				$('#results').html(data);
