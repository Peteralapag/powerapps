...
$pcountdaymode = file($_SERVER['DOCUMENT_ROOT'].'/Modules./DBC_Management/.File/pcountdaysmode.conf');
$linescount = 1;
foreach($pcountdaymode as $linez)
{
	$linescount = $linez;
}


$previousDate = date('Y-m-d', strtotime($transdate. ' -1 day')); // kani diri og action line 1126

$pendongservechecker = $function->GetBranchOrderRecevingServeAllPending($previousDate,$db);

$pcountCheckerPrevious = $function->GetPcountDataInventoryChecker($previousDate,$db);
$pcountChecker = $function->GetPcountDataInventoryChecker($transdate,$db);


if($pcountChecker == 0 && $pcountCheckerPrevious == 1 && $pendongservechecker == 0){
...
if ($pcountChecker == 1) {
    echo '<div class="alert alert-warning alert-sm" role="alert"><i class="fa fa-lock" aria-hidden="true"></i> POSTED.</div>';
} elseif ($pcountCheckerPrevious == 0) {
    echo '<div class="alert alert-success alert-sm" role="alert">The "Post This Summary" button will not appear if there is no previous PCounts.</div>';
} elseif ($pendongservechecker == 1) {
    echo '<div class="alert alert-success alert-sm" role="alert">The "Post This Summary" button will not appear if there are still pending processes for previous dates in Branch Order Receiving.</div>';
}
...
function confirmMe(){
	dialogue_confirm("System Message","Are you sure to post this summary?","warning","PostToSummary","postSummary","")
}
...
function postSummary() {
    ...
    if (dataToSend.length > 0) {
        ...
        $.post("./Modules/DBC_Management/actions/actions.php", { mode: mode, transdate: transdate, data: JSON.stringify(dataToSend) },
            function(data) {        
                $('#smnavdata').html(data);
                rms_reloaderOff();
                viewSummary();
