...
if(isset($_POST['category']) AND !isset($_POST['search']))
{
	...
	if(isset($_POST['search']))
	{
		...
		{
			$category = $_POST['category'];	
			$q = "WHERE recipient='DAVAO BAKING CENTER' AND item_description LIKE '%$search%' OR item_code LIKE '%$search%' OR qr_code LIKE '%$search%' AND  category='$category'";
		} else {
			$q = "WHERE recipient='DAVAO BAKING CENTER' AN item_description LIKE '%$search%' OR item_code LIKE '%$search%' OR qr_code LIKE '%$search%'";
		}		
	} else {
		$q = "WHERE recipient='DAVAO BAKING CENTER' AND active=1 ORDER BY active DESC $limit";
	}
}
?>
<style>
...
function itemlistFormEdit(params,rowid)
{
	...
	$.post("./Modules/DBC_Management/apps/itemlist_form.php", { params: params, rowid: rowid },
	function(data) {		
