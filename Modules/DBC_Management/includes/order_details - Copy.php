...
<div class="order-wrapper" id="orders" style="position:relative">
	<table style="width: 100%" class="lamesako">
		...
		<tbody>
<?php
//	$sqlQuery = "SELECT * FROM dbc_branch_order WHERE control_no='$control_no'";
...
?>		
			<tr>
				...
				<td>
					<input id="aqty<?php echo $x; ?>" <?php echo $stock_text; ?> type="number" class="form-control form-control-sm quantityInput" style="text-align:right" placeholder="<?php echo $actual_quantity; ?>" value="<?php echo $actual_quantity; ?>" onkeyup="test(this.value,'<?php echo $x; ?>','<?php echo $quantity; ?>')">
				</td>
...
function prepRemarks(value,controlno)
{
	...
	$.post("./Modules/DBC_Management/actions/actions.php", { mode: mode, control_no: controlno, remarks: value },
	function(data) {		
		$('#results').html(data);
	});
}
function changeAQ(elemid,rowid,quantity,item_code,transdate)
{
	...
	setTimeout(function()
	{
		$.post("./Modules/DBC_Management/actions/actions.php", { mode: mode, rowid: rowid, actual_quantity: actual_quantity, control_no: control_no, item_code: item_code, transdate: transdate },
		function(data) {		
			$('#results').html(data);
