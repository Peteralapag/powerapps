...
$sqlQuery = "SELECT * FROM dbc_order_request WHERE request_id='$rowid'";
...
if(isset($_POST['editid']) && isset($_POST['params']))
{
	...
	$sqlQueryEditRemarks = "SELECT * FROM dbc_branch_order_remarks WHERE branch='$branch' AND control_no='$mrs_no'";
...
//	var remarks = $('#remarks').val();
	...
	setTimeout(function()
	{
		$.post("./Modules/DBC_Management/actions/create_order_process.php",
		{
			mode: mode,
			editid: editid,
			rowid: rowid,
...
function saveRemarks(params)
{
	...
	setTimeout(function()
	{
		$.post("./Modules/DBC_Management/actions/save_order_remarks.php",
		{
			mode: mode,
			rowid: rowid,
			branch: branch,
...
{
	$.post("./Modules/DBC_Management/includes/request_details_data.php", { rowid: rowid  },
	function(data) {		
		$('#smnavdata').html(data);
	});	
}
