...
if(isset($_SESSION['dbc_appnameuser']))
{
	$app_user = strtolower($_SESSION['dbc_appnameuser']);
...
if ($mode == 'saveChargesEmployee') {
    ...
    $check_stmt = $db->prepare("SELECT COUNT(*) FROM dbc_charges_extension_record WHERE idcode = ? AND chargecodeno = ? AND report_date = ?");
    ...
    $stmt = $db->prepare("INSERT INTO dbc_charges_extension_record 
...
if ($mode == 'deletethisemployeechargeextension') {
    ...
    $sql = "DELETE FROM dbc_charges_extension_record WHERE chargecodeno = ?";
...
if ($mode == 'saveChargesInvolved') {
    ...
    $queryCheckExist = "SELECT COUNT(*) FROM dbc_charges WHERE chargecodeno = ?";
    ...
    if ($count > 0) {
        ...
        $queryDataInsert = "INSERT INTO dbc_charges (chargecodeno, report_date, category, item_description, item_code, quantity, unit_price, total, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($queryDataInsert);
        $stmt->bind_param("sssssdddss", $chargecode, $reportDate, $category, $itemDescription, $itemCode, $quantity, $unitprice, $total, $date_time, $user);
        if ($stmt->execute()) {
...
if($mode == 'addthisemployeechargeextension'){
    ...
    $check_sql = "SELECT COUNT(*) as count FROM dbc_charges_extension_record WHERE chargecodeno = ? AND idcode = ?";
    ...
    if ($check_stmt) {
        ...
        if ($count == 0) {
            $sql = "INSERT INTO dbc_charges_extension_record (chargecodeno, report_date, idcode, employee_name, employee_type, created_by, category, item_description, item_code, unit_price) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
if($mode == 'voidthisbranchorder')
{
	...
	$queryDataUpdate = "UPDATE dbc_order_request SET status='Void', order_remarks='$reasons', void_by='$app_user' WHERE control_no='$control_no' AND order_received=0";
...
if($mode == 'voidthisretouch'){
	...
	$rowid = $_POST['rowid'];
	$itemcode = $_POST['itemcode'];
	$retouchquantity = $function->Getfromthetable('quantity','dbc_retouch',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_retouch',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
...
*/	
	$QUERY = "UPDATE dbc_retouch SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
...
if($mode == 'saveRetouch'){
    ...
    $_SESSION['DBC_TRANSDATE'] = $reportDate;    
    ...
    $queryDataInsert = "INSERT INTO dbc_retouch (report_date, branch, category, item_description, item_code, quantity, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
...
if($mode == 'voidthisrnd'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_rnd',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_rnd',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_rnd SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/	
    ...
    $queryDataInsert = "INSERT INTO dbc_rnd (report_date, branch, category, item_description, item_code, quantity, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user);
    if ($stmt->execute()) {
...
if($mode == 'voidthischarges'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_charges',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_charges',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_charges SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/	
    ...
    $queryDataInsert = "INSERT INTO dbc_charges (report_date, idcode, employee_name, category, item_description, item_code, quantity, unit_price, total, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("ssssssdddss", $reportDate, $idcode, $employeename, $category, $itemDescription, $itemCode, $actualYield, $unitprice, $total, $date_time, $user);
    if ($stmt->execute()) {
...
if($mode == 'voidthisbadorder'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_badorder',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_badorder',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_badorder SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/    
    ...
    $queryDataInsert = "INSERT INTO dbc_badorder (report_date, branch, category, item_description, item_code, quantity, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user);
    if ($stmt->execute()) {
...
if($mode == 'voidthiscomplementary'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_complementary',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_complementary',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_complementary SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/   
    ...
    $queryDataInsert = "INSERT INTO dbc_complementary (report_date, branch, category, item_description, item_code, quantity, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user);
    if ($stmt->execute()) {
...
if($mode == 'voidthisdamage'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_damage',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_damage',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_damage SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/	
    ...
    $queryDataInsert = "INSERT INTO dbc_damage (report_date, branch, category, item_description, item_code, quantity, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user);
    if ($stmt->execute()) {
...
if($mode == 'voidthisreturn'){
	...
	$rowid = $_POST['rowid'];
	$itemcode = $_POST['itemcode'];
	$returnquantity = $function->Getfromthetable('quantity','dbc_return',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_return',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_return SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/
    ...
    $queryDataInsert = "INSERT INTO dbc_return (report_date, branch, category, item_description, item_code, quantity, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user);
    if ($stmt->execute()) {
...
if ($mode == 'datelockChecker') {
    ...
    if ($function->dateLockCheckingExist($transdate, $db) == '1') {
        ...
        $QUERY = "UPDATE dbc_datelock_checker SET status='$checkboxValue', updated_by='$app_user', updated_date='$date_time' WHERE report_date='$transdate'";
    } else {

        $QUERY = "INSERT INTO dbc_datelock_checker (report_date, status, created_by, date_created) VALUES ('$transdate', '$checkboxValue', '$app_user', '$date_time')";
    }
...
if ($mode == 'pcountposting') {
    ...
    foreach ($dataToSend as $row) {
		...
		$queryDataInsert = "INSERT INTO dbc_inventory_pcount (item_code, category, item_description, trans_date, p_count)
                            VALUES (?, ?, ?, ?, ?)";
        $stmt = $db->prepare($queryDataInsert);
...
*/   
    $queryDataInsert = "INSERT INTO dbc_dbc_production (created_time, category, item_description, item_code, batch_received, quantity_received, received_date, report_date, date_created, posted_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
        ...
        if ($stmtStock->execute()) {
            ...
            if ($stmtProduction) {
                ...
                if ($stmtProduction->execute()) {
                    ...
                    $queryDataUpdate = "INSERT INTO dbc_fgts_pcount (item_code,category,item_description,trans_date,pcount,receiving_detail_id,remarks,date_created,posted_by) VALUES ('$itemcode','$category','$itemdescription','$reportdate','$pcount','$id','$remarks','$date_time','$user')";
					if ($db->query($queryDataUpdate) === TRUE)
					{
						echo '<script>
								searchMe();
...
if($mode == 'reopenclosedorderrequest')
{
	...
	$query = "SELECT * FROM dbc_request WHERE control_no='$control_no' AND flag=0"; 
	...
	/*
	$query = "SELECT * FROM dbc_inventory_records WHERE year = ? AND month = ?";
...
if($mode == 'grandfdsrequest')
{
	...
	$queryDataUpdate = "UPDATE dbc_request SET granted_by='$app_user',granted_date='$date_time', flag=1 WHERE id='$rowid'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
...
{
	$query = "SELECT * FROM dbc_branch_order WHERE control_no='$control_no'";
	$results = mysqli_query($db, $query);    
	if ( $results->num_rows > 0 ) 
	{
	    while($ROW = mysqli_fetch_array($results))  
...
if($mode == 'reopenorderrequest')
{
	...
	if ( $results->num_rows > 0 ) 
	{
		...
		$queryInsert = "INSERT INTO dbc_request ($column) VALUES ($insert)";
...
if($mode == 'updateleadtime')
{
	...
	$queryDataUpdate = "UPDATE dbc_inventory_leadtime SET average_leadtime='$minLT', max_leadtime='$maxLT' WHERE leadtime_id=1";
	if ($db->query($queryDataUpdate) === TRUE)
	{
...
{
	$_SESSION['DBC_REPORT_BRANCH'] = $_POST['branch'];
}
if($mode == 'loadbranch')
{
...
if($mode == 'setbranch')
{
	$_SESSION['DBC_BRANCH'] = $_POST['branch'];
}
if($mode == 'setcluster')
{
	$_SESSION['DBC_CLUSTER'] = $_POST['cluster'];
}
if($mode == 'undopcount')
{
	...
	$itemcode = $_POST['itemcode'];
	$elemid = $_POST['elemid'];
	$sqlQueryStk = "SELECT * FROM dbc_inventory_stock WHERE item_code='$itemcode' AND stock_before_pcount_date='$transdate'";
	echo $sqlQueryStk;
    ...
    if ($stkResults->num_rows > 0)
    {
		...
		$queryDataUpdate = "UPDATE dbc_inventory_stock SET stock_in_hand='$stock_before_pcount' WHERE inventory_id='$rowid'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
	    	$qDelete = "DELETE FROM dbc_inventory_pcount WHERE trans_date='$transdate' AND item_code='$itemcode'";
...
if($mode == 'saveexpdate')
{
    ...
    if($results->num_rows > 0)
    {
    	$queryDataUpdate = "UPDATE dbc_inventory_pcount SET expiration_date='$expdate' WHERE trans_date='$transdate' AND item_code='$itemcode'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			echo $db->error;
...
if($mode == 'saveremarks')
{
    ...
    if($results->num_rows > 0)
    {
    	$queryDataUpdate = "UPDATE dbc_inventory_pcount SET remarks='$remarks' WHERE trans_date='$transdate' AND item_code='$itemcode'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			echo $db->error;
...
if($mode == 'savepcount')
{
    ...
    if($results->num_rows > 0)
    {
    	$queryDataUpdate = "UPDATE dbc_inventory_pcount SET p_count='$phycount' WHERE trans_date='$transdate' AND item_code='$itemcode'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			echo $db->error;
...
if($mode == 'setintransit')
{
	...
	{
	    $date = date("Y-m-d");
	    $queryDataUpdate = "UPDATE dbc_order_request SET order_transit=1, status='In-Transit',order_transit_date=? WHERE control_no=? ORDER BY request_id DESC";
	...
	function insertToInventory($year, $month, $day, $item_code, $unit_price, $item_description, $actual_quantity, $control_no, $db)
	{
		...
		$queryInsert = "INSERT INTO dbc_inventory_records ($column) VALUES ($insert)";
		if ($db->query($queryInsert) === TRUE)
		{
	        updateLogistics($control_no, $db);
	        print_r('
	...
	{
	    $queryDataUpdate = "UPDATE dbc_inventory_stock SET stock_in_hand=? WHERE item_code=?";
	    $stmt = $db->prepare($queryDataUpdate);
	...
	$QUERY = "SELECT * FROM dbc_branch_order WHERE control_no=?";
	$stmt = $db->prepare($QUERY);
...
if($mode == 'savelogisticinfo')
{
	...
	$queryDataUpdate = "UPDATE dbc_order_request SET delivery_date=?, delivery_driver=?, plate_number=? WHERE control_no=?";
	$stmt = $db->prepare($queryDataUpdate);
...
if($mode == 'forwardtologistics')
{
	...
	$queryDataUpdate = "UPDATE dbc_order_request SET logistics=1, dr_number='$dr_number' WHERE control_no='$control_no'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
		$queryDataUpdate = "UPDATE dbc_form_numbering SET dr_number='$new_dr_no' WHERE id=1";
		if ($db->query($queryDataUpdate) === TRUE)
		{
...
if($mode == 'preparatorremarks')
{
	...
	$sqlQuery = "SELECT * FROM dbc_branch_order_remarks WHERE control_no='$control_no'";
	$results = mysqli_query($db, $sqlQuery);
	if ( $results->num_rows > 0 ) 
    {
		$queryDataUpdate = "UPDATE dbc_branch_order_remarks SET preparator_remarks='$remarks' WHERE control_no='$control_no'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
	...
	{	
		$queryInsert = "INSERT INTO dbc_branch_order_remarks (`preparator_remarks`,`control_no`) VALUES ('$remarks','$control_no')";
		if ($db->query($queryInsert) === TRUE)
		{
		} else {
			echo $db->error;
...
*/	
	$queryDataUpdate = "UPDATE dbc_branch_order SET wh_quantity='$actual_quantity', actual_quantity='$actual_quantity' WHERE id='$rowid'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
		print_r('
			<script>
...
if($mode == 'preparebranchorder')
{
	...
	$module = $_POST['module'];
	$control_no = $_POST['control_no'];
	$queryDataUpdate = "UPDATE dbc_order_request SET order_preparing=1, order_preparing_by='$app_user', order_preparing_date='$date' WHERE control_no='$control_no' AND order_preparing=0";
	if ($db->query($queryDataUpdate) === TRUE)
	{
...
if($mode == 'receivebranchorder')
{
	...
	$control_no = $_POST['control_no'];
	$module = $_POST['module'];
	$queryDataUpdate = "UPDATE dbc_order_request SET order_received=1, order_received_by='$app_user', order_received_date='$date' WHERE control_no='$control_no' AND order_received=0";
	if ($db->query($queryDataUpdate) === TRUE)
	{
