...
$_SESSION['DBC_TABLE'] = $tableName;
if(isset($_SESSION['DBC_INVSUBMENU']))
{
	$inv_submenu = $_SESSION['DBC_INVSUBMENU'];
} else {
	$inv_submenu = 'inventory_status';
}
if(isset($_SESSION['DBC_ITEMCATEGORY']))
{
	$inv_cat = $_SESSION['DBC_ITEMCATEGORY'];
} else {
	$inv_cat = '';
}
if(isset($_SESSION['DBC_MONTH']))
{
	$inv_month = $_SESSION['DBC_MONTH'];
} else {
	$inv_month = date("m");
}
if(isset($_SESSION['DBC_YEAR']))
{
	$inv_year = $_SESSION['DBC_YEAR'];
} else {
	$inv_year = date("Y");
}
...
function GenerateExcel(table)
{
	...
	if(sessionStorage.stringKo === undefined || sessionStorage.stringKo === null)
	{
		...
		{
			console.log(table);
			$("#genxcel_page").attr("src", "./Modules/DBC_Management/reporting/generate_excel_inventory_status.php?table=" + table + "&columns=" + resultString + "&month=" + month + "&year=" + year + "&limit=" + limit + "&category=" + category);
		}
		else if($('#subinvstatus').val() == 'inventory_monthly')
		{
			$("#genxcel_page").attr("src", "./Modules/DBC_Management/reporting/generate_excel_inventory_monthly.php?table=" + table + "&columns=" + resultString + "&month=" + month + "&year=" + year + "&limit=" + limit + "&category=" + category);
		}
		$("#genxcel").show();
