...
?>	
		...
		<?php
			$total = 0;
			$positiveTotal = 0;			
//			$sqlQueryIR = "SELECT * FROM dbc_branch_order WHERE DATE_FORMAT(delivery_date, '%Y-%m') = '$ddate' AND item_code='$itemcode' AND branch='$branch'";
			$sqlQueryIR = "SELECT * FROM dbc_branch_order WHERE control_no IN ( SELECT control_no FROM dbc_order_request WHERE status = 'Closed' ) AND DATE_FORMAT(delivery_date, '%Y-%m') = '$ddate' AND item_code='$itemcode' AND branch='$branch'";
