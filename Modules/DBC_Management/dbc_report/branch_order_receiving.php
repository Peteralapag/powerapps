...
<div class="subpage-wrapper">
	...
	<!-- Branch Checkbox (Default Checked) -->
	<div class="cntr">
	...
	<!-- Branch Input (Visible by Default) -->
	...
	<!-- Cluster Input (Hidden by Default) -->
...
function searchItem() {
	...
	$.post("./Modules/DBC_Management/dbc_report/search_processor.php", {
		...
		let loadCount = 0; // Counter para malaman kung tapos na lahat ng .load()

		function checkIfAllLoaded() {
			loadCount--;
			if (loadCount === 0) {
				rms_reloaderOff(); // Tatanggalin lang kapag tapos na lahat
			}
		}

		if (data.branch_page) {
			loadCount++;
			$("#iwd").load("./Modules/DBC_Management/dbc_report/" + data.branch_page, {
				selectedbranch: branchInput,
				dateFrom: dateFrom,
				dateTo: dateTo
			}, checkIfAllLoaded);
		...
		if (data.cluster_page) {
			loadCount++;
			$("#iwd").load("./Modules/DBC_Management/dbc_report/" + data.cluster_page, {
		...
		if (data.allbranch_page) {
			loadCount++;
			$("#iwd").load("./Modules/DBC_Management/dbc_report/" + data.allbranch_page, {
		...
		// Kung walang kahit isang load, patayin agad ang loader
		if (loadCount === 0) {
			rms_reloaderOff();
		}
	});
