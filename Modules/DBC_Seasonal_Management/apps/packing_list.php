...
';
	$chkQuery = "
		SELECT pl.*
		FROM dbc_seasonal_picklist pl
		WHERE EXISTS (
		    SELECT 1
		    FROM dbc_seasonal_branch_order bo
...
';
		/* ####################################################################################################### */			
			$query = "SELECT * FROM dbc_seasonal_picklist WHERE control_no='$control_no' AND trans_date='$trans_date'";
