...
<!--div>
	<input type="text" class="form-control" value="" placeholder="Search branch here" onkeyup="searchBranchHere(this.value)">
</div-->
...
function searchBranchHere(branch){
	...
	$.post("./Modules/DBC_Seasonal_Management/apps/set_branch_active_form.php", { branch: branch },
	function(data) {
		$('#results').html(data);
		rms_reloaderOff();
	});
...
function addThisBrachActive(branchid){
	...
	rms_reloaderOn('Loading...');
	$.post("./Modules/DBC_Seasonal_Management/actions/actions.php", { mode: mode, branchid: branchid, branch: branch, branchvalue: branchvalue },
	function(data) {
		$('#results').html(data);
		rms_reloaderOff();
	});
