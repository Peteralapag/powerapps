...
class DBCFunctions
{
	...
	public function GetTransferOutDataVer2($itemcode,$transdatefrom,$transdateto,$db)
	{
		
		$QUERY = "SELECT SUM(actual_quantity) AS QUANTITY FROM dbc_seasonal_branch_mrs_transaction WHERE item_code='$itemcode' AND trans_date BETWEEN '$transdatefrom' AND '$transdateto' AND control_no IN (SELECT control_no FROM dbc_seasonal_order_request_generate_dr WHERE status = 'Closed' AND order_delivered = 1)";
	...
	public function detectItemIdIfExiist($controlno,$itemcode,$db)
	{

		$query = "SELECT * FROM dbc_seasonal_branch_order WHERE control_no='$controlno' AND item_code='$itemcode'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
			return 1;		    
	...
	function updateFinalizeCtrlNo($branch, $controlno, $db)
	{
	    $queryDataUpdate = "UPDATE dbc_seasonal_order_request SET partial_status = 1 WHERE branch = ? AND control_no = ?";
	    $stmt = $db->prepare($queryDataUpdate);
	    if (!$stmt) {
	        echo '
	            <script>
	                swal("Preparation Error:", "' . $db->error . '", "error");
	            </script>
	        ';
	        return;
	    }
	...
	{
	    $queryDataUpdate = "UPDATE dbc_seasonal_order_request 
	                        SET status = 'In-Transit', logistics = 1 , order_transit=1, finalize =1
	                        WHERE request_id = ?";
	...
	public function updateLogistics($rowid, $controlno, $db)
	{
		$date = date("Y-m-d");
	    $queryDataUpdate = "UPDATE dbc_seasonal_order_request_generate_dr SET order_transit=1, status='In-Transit',order_transit_date=? WHERE id=? ORDER BY request_id DESC";
	    $stmt = $db->prepare($queryDataUpdate);
	    $stmt->bind_param("si", $date, $rowid);	   
	 	if ($stmt->execute()) {
	        print_r('
	            <script>		
	                dr_details_ver2("' . $rowid . '","' . $controlno . '");
	            </script>
	        ');
	    } else {
	        echo $stmt->error;
	...
	public function getItemsViaTable($table,$column,$requestid,$itemcode,$db)
	{
		$QUERY = "SELECT * FROM $table WHERE request_id='$requestid' AND item_code='$itemcode'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	public function UpdateBranchOrderDeliveryDatever2($rowid, $delivery_date, $db)
	{
		
	    $stmt = $db->prepare("UPDATE dbc_seasonal_branch_mrs_transaction 
	                          SET delivery_date = ? 
	                          WHERE request_id = ?");
	    if (!$stmt) {
	        return $db->error;
	    ...
	    $stmt->bind_param("si", $delivery_date, $rowid);
	...
	public function getSumRemainingBalance($controlno, $itemcode, $db)
	{
	    ...
	    $query = "SELECT id FROM dbc_seasonal_order_request_generate_dr 
	              WHERE control_no = ? AND (status = 'In-Transit' OR status = 'Closed') AND logistics = 1";
	    ...
	    if ($stmt) {
	        ...
	        if ($result->num_rows > 0) {
	            while ($row = $result->fetch_assoc()) {
	                ...
	                $query2 = "SELECT SUM(wh_quantity) AS TOTAL 
	                           FROM dbc_seasonal_branch_mrs_transaction 
	                           WHERE request_id = ? AND item_code = ?";
	                $stmt2 = $db->prepare($query2);
	
	                if ($stmt2) {
	                    $stmt2->bind_param("ss", $rowid, $itemcode);
	                    $stmt2->execute();
	                    $result2 = $stmt2->get_result();
	
	                    if ($result2->num_rows > 0) {
	                        $row2 = $result2->fetch_assoc();
	                        $val += $row2['TOTAL'] ?? 0; // Add up totals for multiple rows
	                    }
	                    $stmt2->close();
	                } else {
	                    error_log("Failed to prepare second query: " . $db->error);
	                    throw new Exception("Error preparing second query: " . $db->error);
	                }
	            }
	        } else {
	            error_log("No matching rows found in first query.");
	        }
	        $stmt->close();
	    } else {
	        error_log("Failed to prepare first query: " . $db->error);
	        throw new Exception("Error preparing first query: " . $db->error);
	    }
	...
	public function DrCheckinOpenExist($controlno,$db)
	{
		$QUERY = "SELECT * FROM dbc_seasonal_order_request_generate_dr WHERE control_no = '$controlno' AND status = 'Open'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	public function getSumQuantityItem($table1,$column,$requestid,$itemcode,$db)
	{
		$table = 'dbc_seasonal_'.$table1;
		$QUERY = "SELECT SUM($column) AS TOTAL FROM $table WHERE request_id='$requestid' AND item_code='$itemcode' AND status='submitted'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	public function getvalueofseasonal($column,$rowid,$itemcode,$db)
	{
		$retVal = '';
		$query = "SELECT * FROM dbc_seasonal_branch_mrs_transaction WHERE request_id='$rowid' AND item_code='$itemcode'";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
		    while($ROW = mysqli_fetch_array($results))  
	...
	public function GetOrderStatusViaID($requestid,$column,$db)
	{
		$retVal = 0;
	...
	public function getTransactionId($controlno, $transactionid, $db)
	{
	    
		$QUERY = "SELECT * FROM dbc_seasonal_branch_mrs_transaction WHERE control_no='$controlno' AND transaction_id='$transactionid'";
	...
	{
	    $query = "SELECT item_description, report_date, posted_by FROM dbc_seasonal_dbc_production WHERE status = 'No' ORDER BY report_date DESC";
	...
	public function GetTransferOutDataViaBranchReceived($branch,$itemcode,$transdate,$db)
	{
	    ...
	    $QUERY = "SELECT SUM(actual_quantity) AS QUANTITY 
	              ...
	              AND control_no IN (
	                  SELECT control_no 
	                  FROM dbc_seasonal_order_request 
	                  WHERE status = 'Closed' 
	...
	function countDetectEmployeeChargedBAKERONLY2($module, $chargeno, $db) {
	    ...
	    $query = "SELECT SUM($module) as total FROM dbc_seasonal_charges_extension_record WHERE chargecodeno = ? AND employee_type = 'BAKER'";
	...
	function countDetectEmployeeChargedNONEBAKERONLY($module, $chargeno, $db) {
	    ...
	    $query = "SELECT SUM($module) as total FROM dbc_seasonal_charges_extension_record WHERE chargecodeno = ? AND employee_type = 'NONE BAKER'";
	...
	function updateChargesByChargedBAKERONLY($chargeno, $new_quantity, $new_total, $db) {
	    ...
	    $stmt = $db->prepare("UPDATE dbc_seasonal_charges_extension_record SET quantity = ?, total = ? WHERE chargecodeno = ? AND employee_type = 'BAKER'");
	...
	function countDetectEmployeeChargedBAKERONLY($chargeno, $db) {
	    ...
	    $stmt = $db->prepare("SELECT COUNT(*) as record_count FROM dbc_seasonal_charges_extension_record WHERE chargecodeno = ? AND employee_type = 'BAKER'");
    	$stmt->bind_param("s", $chargeno);	
	...
	function updateChargesByChargeCode($chargeno, $new_quantity, $new_total, $db) {
	    ...
	    $stmt = $db->prepare("UPDATE dbc_seasonal_charges_extension_record SET quantity = ?, total = ? WHERE chargecodeno = ?");
	    $stmt->bind_param("dds", $new_quantity, $new_total, $chargeno);
...
//	        return "Error updating record: " . $db->error;
	...
	function countDetectEmployeeCharged($chargeno, $db) {
	    ...
	    $stmt = $db->prepare("SELECT COUNT(*) as record_count FROM dbc_seasonal_charges_extension_record WHERE chargecodeno = ?");
	...
	public function getValueSummaryviadate($datefrom, $dateto, $itemcode, $db)
	{
	    $QUERY = "
	        ...
	        FROM dbc_seasonal_branch_order b
	        JOIN dbc_seasonal_order_request o ON b.control_no = o.control_no
	...
	{
		$sqlQueryStk = "SELECT * FROM dbc_seasonal_charges_extension_record WHERE chargecodeno = '$chargecodeno'";
	...
	{
		$returnquantity = $this->Getfromthetable('quantity','dbc_seasonal_retouch',$rowid,$itemcode,$db);
		...
		$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand-'$returnquantity', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
	...
	public function getIntoDbcTblitemslist($datefrom, $dateto, $db)
	{
	    ...
	    $branches = [];
	    $branchQuery = "SELECT * FROM dbc_seasonal_tbl_branch WHERE status = 1";  
	    ...
	    $query = "SELECT * FROM dbc_seasonal_itemlist WHERE active = 1 ORDER BY ordered ASC";  
	    $result = mysqli_query($db, $query);  
	    $i = 1;
	...
	{
		$query ="SELECT * FROM dbc_seasonal_tbl_branch WHERE status = 1";  
		$result = mysqli_query($db, $query);  
		while($ROWS = mysqli_fetch_array($result))  
		{
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_tbl_branch WHERE branch_id='$branchid'";
		$RESULTS = mysqli_query($db, $QUERY);
	...
	public function insertIntoDbcTblBranchTable($branch, $branchid, $active, $db)
	{
	    ...
	    $checkQuery = "SELECT * FROM dbc_seasonal_tbl_branch WHERE branch_id = ?";
	    ...
	    if ($result->num_rows > 0) {
	        ...
	        $deleteQuery = "DELETE FROM dbc_seasonal_tbl_branch WHERE branch_id = ?";
	        ...
	        // Insert new branch
	        $insertQuery = "INSERT INTO dbc_seasonal_tbl_branch (branch_id, branch, status) VALUES (?, ?, ?)";
	...
	{
		$quantity = $this->Getfromthetable('quantity','dbc_seasonal_'.$table,$rowid,$itemcode,$db);
		
		$stockbeforepcountdate = $this->GetInventoryStockStocknHandReturn('stock_before_pcount_date',$itemcode,$db);
		$stockbeforepcount = $this->GetInventoryStockStocknHandReturn('stock_before_pcount',$itemcode,$db);
		
		$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand+'$quantity', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	public function otherinventoryoutdata($itemcode,$itemval,$dateupdated,$updatedby,$db)
	{
		...
		$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand-'$itemval', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	{
		$returnquantity = $this->Getfromthetable('quantity','dbc_seasonal_return',$rowid,$itemcode,$db);
	...
	public function insertReturnData($itemcode,$returnval,$dateupdated,$updatedby,$db)
	{
		...
		$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_before_pcount_date='$stockbeforepcountdate',stock_before_pcount='$stockbeforepcount', stock_in_hand=stock_in_hand+'$returnval', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	{
		$sqlQueryStk = "SELECT $module FROM dbc_seasonal_inventory_stock WHERE item_code='$itemcode'";
	    $stkResults = mysqli_query($db, $sqlQueryStk);
	    $val = 0;
	    if ($stkResults->num_rows > 0)
	    {
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_datelock_checker WHERE report_date='$transdate'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	public function executeInventory($itemcode,$pcount,$transdate,$dateupdated,$updatedby,$db)
	{
		...
		$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_before_pcount_date='$transdate',stock_before_pcount='$valuebeforepcount', stock_in_hand='$pcount', date_updated='$dateupdated',updated_by='$updatedby' WHERE item_code='$itemcode'";
        if ($db->query($queryDataUpdate) === TRUE) {
        } else {
            echo $db->error;
        }
	...
	{
		$sqlQueryStk = "SELECT stock_in_hand FROM dbc_seasonal_inventory_stock WHERE item_code='$itemcode'";
	    $stkResults = mysqli_query($db, $sqlQueryStk);
	    $val = 0;
	    if ($stkResults->num_rows > 0)
	    {
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM dbc_seasonal_dbc_production WHERE item_code = '$itemcode' AND MONTH(report_date) = MONTH('$transdate')";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM dbc_seasonal_inventory_pcount WHERE trans_date='$transdate' AND item_code='$itemcode'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_inventory_pcount WHERE trans_date = '$transdate'";
		$RESULTS = mysqli_query($db, $QUERY);
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	{
		$query ="SELECT * FROM dbc_seasonal_itemlist WHERE item_code='$itemcode'";  
		$RESULTS = mysqli_query($db, $query);
		$val = '';
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	public function getPcountExist($transdate, $db) {
	    ...
	    $query = "SELECT trans_date FROM dbc_seasonal_inventory_pcount WHERE trans_date='$transdate'";
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_dbc_production WHERE item_code='$itemcode' AND report_date='$transdate' AND status <> 'Void'";
		$RESULTS = mysqli_query($db, $QUERY);
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_dbc_production WHERE report_date='$transdate' AND status='Yes'";
		$RESULTS = mysqli_query($db, $QUERY);
		if ( $RESULTS->num_rows > 0 ) 
		{
			return 1;
	...
	public function GetTransferOutData($itemcode,$transdatefrom,$transdateto,$db)
	{
		...
		$QUERY = "SELECT SUM(actual_quantity) AS QUANTITY FROM dbc_seasonal_branch_order WHERE item_code='$itemcode' AND trans_date BETWEEN '$transdatefrom' AND '$transdateto' AND control_no IN (SELECT control_no FROM dbc_seasonal_order_request WHERE status = 'Closed' AND order_delivered = 1)";
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM dbc_seasonal_dbc_production WHERE report_date BETWEEN '$datefrom' AND '$dateto' AND item_code='$itemcode' AND status='Yes'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
		$QUERY = "SELECT SUM($column) AS TOTAL FROM dbc_seasonal_fgts_pcount WHERE trans_date BETWEEN '$datefrom' AND '$dateto' AND item_code='$itemcode'";
		$RESULTS = mysqli_query($db, $QUERY);
		$val = 0;
		if ( $RESULTS->num_rows > 0 ) 
		{
	...
	{
	    $query ="SELECT COUNT(*) AS pendingTotal FROM dbc_seasonal_dbc_production WHERE status='No'";  
	...
	{
		$query ="SELECT * FROM dbc_seasonal_itemlist WHERE active=1";  
		$result = mysqli_query($db, $query);  
	...
	{
		$queryDataUpdate = "UPDATE dbc_seasonal_branch_order SET delivery_date='$delivery_date' WHERE control_no='$control_no'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			return $db->error;
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_classifications_records WHERE item_code='$item_code' AND flag=0";
		$RESULTS = mysqli_query($db, $QUERY);		
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_order_request WHERE item_code='$item_code'";
		$RESULTS = mysqli_query($db, $QUERY);		
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
	...
	public function getDeliveryDateVer2($rowid,$db)
	{
		$QUERY = "SELECT * FROM dbc_seasonal_order_request_generate_dr WHERE id='$rowid'";
		$RESULTS = mysqli_query($db, $QUERY);		
		if ( $RESULTS->num_rows > 0 ) 
	...
	public function getDeliveryMonthVer2($rowid,$db)
	{
		$cur_date = date("m");
		$QUERY = "SELECT * FROM dbc_seasonal_order_request_generate_dr WHERE id='$rowid'";
		$RESULTS = mysqli_query($db, $QUERY);		
	...
	{
		$queryDataUpdate = "UPDATE dbc_seasonal_order_request SET status='Submitted', logistics=0, order_transit=0 WHERE control_no='$control_no'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
			//echo "SUCCESS";
		} else {
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_inventory_stock WHERE item_code='$item_code'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
			...
			/* ------------------------------------------------------------------------------------------- */
				$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_in_hand='$new_stock' WHERE item_code='$item_code'";
				if ($db->query($queryDataUpdate) === TRUE)
				{
				/* ##################################################################### */
					$queryDataUpdate = "UPDATE dbc_seasonal_order_request SET status='Submitted', logistics=0, order_transit=0 WHERE control_no='$control_no'";
	...
	{
		$queryBO = "SELECT * FROM dbc_seasonal_branch_order WHERE control_no='$control_no'";
		...
		$queryPL = "SELECT * FROM dbc_seasonal_picklist WHERE control_no='$control_no'";
	...
	{
		$QUERY = "SELECT * FROM dbc_seasonal_receiving WHERE receiving_id='$receiving_id'";
		$RESULTS = mysqli_query($db, $QUERY);    
		if ( $RESULTS->num_rows > 0 ) 
		{
			while($ROW = mysqli_fetch_array($RESULTS))  
	...
	{
		$query = "SELECT * FROM dbc_seasonal_receiving WHERE receiving_id='$receiving_id' AND status='Closed'";
		$result = mysqli_query($db, $query);		
	    if ( $result->num_rows > 0 ) 
	    {
	    	return 1;
	...
	{
		$queryBO = "SELECT * FROM dbc_seasonal_branch_order WHERE control_no='$control_no' AND branch='$branch'";
		$resultBO = mysqli_query($db, $queryBO);		
		if ($resultBO)
		{
		    $boCount = mysqli_num_rows($resultBO);
	...
	{
		$query = "SELECT * FROM dbc_seasonal_reporting_option WHERE active=1 ORDER BY ordering ASC";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	public function DoAuditLogs($log_date,$activity,$log_by,$db)
	{
		...
		$queryInsert = "INSERT INTO dbc_seasonal_audit_logs ($column) VALUES ($insert)";
	...
	{
		$query = "SELECT * FROM dbc_seasonal_item_location WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
	...
	{
		$query = "SELECT * FROM dbc_seasonal_inventory_pcount WHERE item_code='$itemcode' AND trans_date='$transdate'";
		$results = $db->query($query);			
	    if($results->num_rows > 0)
	    {
		    while($ROW = mysqli_fetch_array($results))  
	...
	{
		$query = "SELECT * FROM dbc_seasonal_inventory_records WHERE item_code='$itemcode' AND month='$month' AND year='$year'";
		$results = $db->query($query);			
	    if($results->num_rows > 0)
	    {
		    while($ROW = mysqli_fetch_array($results))  
	...
	{
		$query = "SELECT * FROM dbc_seasonal_reporting_option_sub WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
			$return = '';
	...
	{
		$query = "SELECT * FROM dbc_seasonal_reporting_option WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
...
//		exit();
	...
	{
		$query = "SELECT * FROM dbc_seasonal_recipient WHERE active=1";
		$results = mysqli_query($db, $query);    
		if ( $results->num_rows > 0 ) 
		{
