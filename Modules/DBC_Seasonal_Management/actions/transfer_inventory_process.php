...
function insertToReceiving($rowid,$supplier_id,$po_no,$si_no,$itemcode,$received_by,$received_date,$data,$quantity_received,$takeout_amount,$item_code,$function,$db)
{
	...
	$queryInsert = "INSERT INTO dbc_seasonal_receiving ($column) VALUES ($insert)";
...
function insertToDetails($receiving_id,$rowid,$data,$itemcode,$quantity_received,$takeout_amount,$item_code,$function,$db)
{
	...
	$queryInsert = "INSERT INTO dbc_seasonal_receiving_details ($column) VALUES ($insert)";
	if ($db->query($queryInsert) === TRUE)
	{
...
function executeInventory($itemcode,$rowid,$quantity_received,$takeout_amount,$item_code,$function,$db)
{
    ...
    $sqlQueryChk = "SELECT * FROM dbc_seasonal_warehouse_transfer WHERE itemcode='$itemcode'";
    echo $sqlQueryChk;
    ...
    {
		$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_in_hand='$new_stock' WHERE item_code='$itemcode'";
...
{
	$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_in_hand='$deduct_stock' WHERE item_code='$item_code'";
    if ($db->query($queryDataUpdate) === TRUE) {
        closeTransfer($rowid, $itemcode, $db); // Pass rowid and itemcode as a parameters
    } else {
        echo $db->error;
...
{
    $queryDataUpdate = "UPDATE dbc_seasonal_warehouse_transfer SET status='Closed' WHERE id='$rowid'";
    if ($db->query($queryDataUpdate) === TRUE) {
        print_r('
            <script>
                stockList();
