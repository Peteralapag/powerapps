...
if($mode=='c10cc6b684e1417e8ffa924de1e58373')
{
	...
	$pass = $_POST['password'];
	$user_app = "DBC Seasonal Management";	
    ...
    { 
		...
		if($void == 0)
		{
			echo checkDBCManager($idcode,$db);
			if(checkDBCManager($idcode,$db) != '')		
			{
				$_SESSION['dbc_seasonal_user_recipient'] = checkDBCManager($idcode,$db);
			}
			$_SESSION['dbc_seasonal_username'] = $username;
			$_SESSION['dbc_seasonal_company'] = $company;
			$_SESSION['dbc_seasonal_appnameuser'] = $employee;
			$_SESSION['dbc_seasonal_userlevel'] = $user_level;
			$_SESSION['dbc_seasonal_userrole'] = $user_role;
			$_SESSION['dbc_seasonal_application'] = $user_app;
			$_SESSION['DBC_SEASONAL_TRANSDATE'] = $formattedDate;	
			$cmd = '';
			$cmd .='				
				<script>
					$("#formmodal").hide();
...
function checkDBCManager($idcode,$db)
{
	$checkManager = "SELECT * FROM dbc_seasonal_user_recipient WHERE idcode='$idcode'";
