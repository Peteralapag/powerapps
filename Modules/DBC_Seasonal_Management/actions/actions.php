...
if(isset($_SESSION['dbc_seasonal_appnameuser']))
{
	$app_user = strtolower($_SESSION['dbc_seasonal_appnameuser']);
...
if (!isset($_SESSION['dbc_seasonal_appnameuser'])) {      
    echo '<script>
    		app_alert("System Message", "User session not set. Operation aborted", "warning");
    	</script>';
    exit();
}
...
if($mode == 'savelogisticinfover2')
{
	...
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request_generate_dr SET delivery_date=?, delivery_driver=?, plate_number=? WHERE id=?";
	$stmt = $db->prepare($queryDataUpdate);
	$stmt->bind_param("ssss", $delivery_date, $delivery_driver, $plate_number, $rowid);
	if ($stmt->execute())
	{
		$function->UpdateBranchOrderDeliveryDatever2($rowid,$delivery_date,$db);
	    print_r('
	        <script>
	            swal("Success", "Logistic Information has been added to the Delivery Receipt", "success");
	            dr_details_ver2("'. $rowid.'", "'.$control_no.'");
...
if($mode == 'setintransit_ver2')
{
	
	$controlno = $_POST['control_no'];	
	$rowid = $_POST['rowid'];		

	$delivery_date = $function->GetOrderStatusSecondTable($rowid, 'delivery_date', $db);
	$branch = $function->GetOrderStatusSecondTable($rowid, 'branch', $db);
	
    $year = date("Y", strtotime($delivery_date));
    $month = date("m", strtotime($delivery_date));
    $day = date("d", strtotime($delivery_date));
    
    $function->updateLogistics($rowid, $controlno, $db);
    
	$requestid = $function->GetOrderStatusSecondTable($rowid, 'request_id', $db);
	$partialstatus = $function->orderRequestMotherTable($requestid,'partial_status',$db);
	
	$finalize = $function->GetOrderStatus($controlno, 'finalize', $db);
	
	
	if($partialstatus == 1){
		$function->updatemainOrder($requestid, $db);
	}
	

	$QUERY = "SELECT * FROM dbc_seasonal_branch_mrs_transaction WHERE request_id=?";
	...
	while ($ROW = $RESULTS->fetch_assoc())
	{
	    ...
	    $actual_quantity = $ROW['wh_quantity'];
	...
	print_r('
			<script>
				swal("System Message", "Order In-Transit", "success");	
			</script>
		');
		
	$log_msg = 'Branch Order Receiving | Set In-Transit | '.$controlno;
...
if($mode == 'voidthisseasonaltransaction')
{
	$app_user = ucwords($app_user);
	$control_no = $_POST['controlno'];
	$module = 'Void Seasonal Transaction';
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request_generate_dr SET status='Void', void_by='$app_user' WHERE control_no='$control_no' AND order_received=0";
...
if($mode == 'forwardtologisticsver2')
{
	...
	$module = $_POST['module'];
	$rowid = $_POST['rowid'];
	$allRemaining = $_POST['allRemaining'];
	...
	if ($allRemaining == 1) {
		...
		$function->updateFinalizeCtrlNo($branch, $control_no, $db);
	}
	
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request_generate_dr SET status='Submitted', logistics=1, dr_number='$dr_number' WHERE id='$rowid'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
		$queryDataUpdate = "UPDATE dbc_seasonal_form_numbering SET dr_number='$new_dr_no' WHERE id=1";
		if ($db->query($queryDataUpdate) === TRUE)
		{
...
if($mode == 'generateTransactionId') {

    $controlno = $_POST['controlno'];
    $branch = $_POST['branch'];
    $transactionid = $_POST['transactionid'];

    $control_no = $function->GetOrderStatus($controlno, 'control_no', $db);
    
    $requestid = $function->GetOrderStatus($controlno, 'request_id', $db);
    
    $trans_date = $function->GetOrderStatus($controlno, 'trans_date', $db);
    $cluster = $function->GetOrderStatus($controlno, 'cluster', $db);
    $recipient = $function->GetOrderStatus($controlno, 'recipient', $db);
    $created_by = $function->GetOrderStatus($controlno, 'created_by', $db);
    $priority = $function->GetOrderStatus($controlno, 'priority', $db);
    $date_user = $function->GetOrderStatus($controlno, 'date_created', $db);
    
    $openTransaction = $function->DrCheckinOpenExist($control_no,$db);
	...
	if($openTransaction == 1){
		echo '<script>
        		app_alert("System Message", "An open transaction must be completed first", "warning");
        	</script>';
        exit();
	}

	$partialstatus = $function->GetOrderStatus($controlno, 'partial_status', $db);
	if($partialstatus == 1){
		echo '<script>
        		app_alert("System Message", "This MRS No. '.$control_no.' Already Closed", "warning");
        	</script>';
        exit();
	}
    ...
    $columns = "`request_id`,`control_no`, `trans_date`, `cluster`, `branch`, `recipient`, `created_by`, `priority`, `date_created`, `transaction_id`";
    $values = "'$requestid','$control_no', '$date', '$cluster', '$branch', '$recipient', '$created_by', '$priority', '$date_time', '$transactionid'";

    $queryInsert = "INSERT INTO dbc_seasonal_order_request_generate_dr ($columns) VALUES ($values)";
    $result = mysqli_query($db, $queryInsert);

    if ($result) {
        echo '<script>
        		app_alert("System Message", "Transaction ID generated successfully!", "success");
        		
        		
        		
        		viewIncompleteTransaction("'.$control_no.'","'.$transactionid.'","'.$requestid.'","'.$branch.'");
        	</script>';

    } else {
        echo "Error generating Transaction ID: " . mysqli_error($db);
    }
}
...
if ($mode == 'partialquantitysaving') {

    $rowid = $_POST['rowid'];
    
    
    $branch = $function->GetOrderStatusSecondTable($rowid, 'branch', $db);
    $controlno = $function->GetOrderStatusSecondTable($rowid, 'control_no', $db);
    $transdate = $function->GetOrderStatusSecondTable($rowid, 'trans_date', $db);

    $idcode = $_POST['idcode'];
    $item = $_POST['item'];
    $uom = $_POST['uom'];
    $reqquantity = $_POST['reqquantity'];
    $partialquantity = $_POST['partialquantity'];
    ...
    if ($checkStmt) {
        $checkStmt->bind_param("ss", $rowid, $idcode);
        $checkStmt->execute();
        ...
        if ($recordExists > 0) {
            ...
            $updateStmt = $db->prepare("
                UPDATE dbc_seasonal_branch_mrs_transaction
                SET wh_quantity = ?
                WHERE request_id = ? AND item_code = ?
            ");
            if ($updateStmt) {
                $updateStmt->bind_param("dss", $partialquantity, $rowid, $idcode);
...
//                    echo "updated successfully.";
            ...
            $insertStmt = $db->prepare("
                INSERT INTO dbc_seasonal_branch_mrs_transaction 
                (request_id, branch, control_no, item_code, item_description, uom, quantity, wh_quantity, created_by, trans_date)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            if ($insertStmt) {
                $insertStmt->bind_param(
                    "ssssssddss",
                    $rowid, $branch, $controlno, $idcode, $item, $uom,
                    $reqquantity, $partialquantity, $app_user, $transdate
                );
...
if ($mode == 'saveChargesEmployee') {
    ...
    $check_stmt = $db->prepare("SELECT COUNT(*) FROM dbc_seasonal_charges_extension_record WHERE idcode = ? AND chargecodeno = ? AND report_date = ?");
    ...
    $stmt = $db->prepare("INSERT INTO dbc_seasonal_charges_extension_record 
...
if ($mode == 'deletethisemployeechargeextension') {
    ...
    $sql = "DELETE FROM dbc_seasonal_charges_extension_record WHERE chargecodeno = ?";
...
if ($mode == 'saveChargesInvolved') {
    ...
    $queryCheckExist = "SELECT COUNT(*) FROM dbc_seasonal_charges WHERE chargecodeno = ?";
    ...
    if ($count > 0) {
        ...
        $queryDataInsert = "INSERT INTO dbc_seasonal_charges (chargecodeno, report_date, category, item_description, item_code, quantity, unit_price, total, created_date, created_by, remarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
if($mode == 'getItemCodeNewModulesCharges'){
    ...
    $query ="SELECT * FROM dbc_seasonal_itemlist WHERE item_description=?";  
    $stmt = $db->prepare($query);
...
if($mode == 'addthisemployeechargeextension'){
    ...
    $check_sql = "SELECT COUNT(*) as count FROM dbc_seasonal_charges_extension_record WHERE chargecodeno = ? AND idcode = ?";
    ...
    if ($check_stmt) {
        ...
        if ($count == 0) {
            $sql = "INSERT INTO dbc_seasonal_charges_extension_record (chargecodeno, report_date, idcode, employee_name, employee_type, created_by, category, item_description, item_code, unit_price) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            $stmt = $db->prepare($sql);

            if ($stmt) {
                $stmt->bind_param("sssssssssd", $chargecodeno, $dateselected, $idcode, $employeename, $employeetype, $encodedby, $category, $itemdescription, $itemcode, $unitprice);
...
if($mode == 'voidthisbranchorder')
{
	...
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request SET status='Void', void_by='$app_user' WHERE control_no='$control_no' AND order_received=0";
	if ($db->query($queryDataUpdate) === TRUE)
	{
		$log_msg = $module.' | Void Order | CN'.$control_no;
		print_r('
...
if($mode == 'voidthisretouch'){
	...
	$rowid = $_POST['rowid'];
	$itemcode = $_POST['itemcode'];
	$retouchquantity = $function->Getfromthetable('quantity','dbc_seasonal_retouch',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_seasonal_retouch',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
...
*/	
	$QUERY = "UPDATE dbc_seasonal_retouch SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
...
if($mode == 'saveRetouch'){
    ...
    $_SESSION['DBC_SEASONAL_TRANSDATE'] = $reportDate;    
    ...
    $queryDataInsert = "INSERT INTO dbc_seasonal_retouch (report_date, branch, category, item_description, item_code, quantity, created_date, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
...
if($mode == 'voidthisrnd'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_seasonal_rnd',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_seasonal_rnd',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_seasonal_rnd SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/	
    ...
    $queryDataInsert = "INSERT INTO dbc_seasonal_rnd (report_date, branch, category, item_description, item_code, quantity, created_date, created_by, remarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
...
if($mode == 'voidthischarges'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_seasonal_charges',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_seasonal_charges',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_seasonal_charges SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
if($mode == 'saveCharges'){
	...
	$unitprice = $function->customemoduletablegetvalue('unit_price','dbc_seasonal_itemlist','item_code',$itemCode,$db);
...
*/	
    ...
    $queryDataInsert = "INSERT INTO dbc_seasonal_charges (report_date, idcode, employee_name, category, item_description, item_code, quantity, unit_price, total, created_date, created_by, remarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
...
if($mode == 'voidthisbadorder'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_seasonal_badorder',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_seasonal_badorder',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_seasonal_badorder SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/    
    ...
    $queryDataInsert = "INSERT INTO dbc_seasonal_badorder (report_date, branch, category, item_description, item_code, quantity, created_date, created_by, remarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdsss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user, $remarks);
    if ($stmt->execute()) {
...
if($mode == 'voidthiscomplementary'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_seasonal_complementary',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_seasonal_complementary',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_seasonal_complementary SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/   
    ...
    $queryDataInsert = "INSERT INTO dbc_seasonal_complementary (report_date, branch, category, item_description, item_code, quantity, created_date, created_by, remarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdsss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user, $remarks);
    if ($stmt->execute()) {
...
if($mode == 'voidthisdamage'){
	...
	$quantity = $function->Getfromthetable('quantity','dbc_seasonal_damage',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_seasonal_damage',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_seasonal_damage SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/	
    ...
    $queryDataInsert = "INSERT INTO dbc_seasonal_damage (report_date, branch, category, item_description, item_code, quantity, created_date, created_by, remarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdsss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user, $remarks);
    if ($stmt->execute()) {
...
if($mode == 'voidthisreturn'){
	...
	$rowid = $_POST['rowid'];
	$itemcode = $_POST['itemcode'];
	$returnquantity = $function->Getfromthetable('quantity','dbc_seasonal_return',$rowid,$itemcode,$db);
	$itemdescription = $function->Getfromthetable('item_description','dbc_seasonal_return',$rowid,$itemcode,$db);
	$stockinhand = $function->GetInventoryStockStocknHandReturn('stock_in_hand',$itemcode,$db);
	...
	$QUERY = "UPDATE dbc_seasonal_return SET status='1', void_date='$date_time', void_by='$app_user' WHERE id='$rowid'";
    if (mysqli_query($db, $QUERY)) {
...
*/
    ...
    $queryDataInsert = "INSERT INTO dbc_seasonal_return (report_date, branch, category, item_description, item_code, quantity, created_date, created_by, remarks)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
    $stmt->bind_param("sssssdsss", $reportDate, $branch, $category, $itemDescription, $itemCode, $actualYield, $date_time, $user, $remarks);
    if ($stmt->execute()) {
...
if ($mode == 'datelockChecker') {
    ...
    if ($function->dateLockCheckingExist($transdate, $db) == '1') {
        ...
        $QUERY = "UPDATE dbc_seasonal_datelock_checker SET status='$checkboxValue', updated_by='$app_user', updated_date='$date_time' WHERE report_date='$transdate'";
    } else {

        $QUERY = "INSERT INTO dbc_seasonal_datelock_checker (report_date, status, created_by, date_created) VALUES ('$transdate', '$checkboxValue', '$app_user', '$date_time')";
    }
...
if ($mode == 'pcountposting') {
    ...
    foreach ($dataToSend as $row) {
		...
		$queryDataInsert = "INSERT INTO dbc_seasonal_inventory_pcount (item_code, category, item_description, trans_date, p_count)
                            VALUES (?, ?, ?, ?, ?)";
        $stmt = $db->prepare($queryDataInsert);
...
if($mode == 'saveFgts'){
    ...
    $_SESSION['DBC_SEASONAL_TRANSDATE'] = $reportDate;
    $_SESSION['DBC_DATEFROM'] = $reportDate;
    $_SESSION['DBC_DATETO'] = $reportDate;
...
*/   
    $queryDataInsert = "INSERT INTO dbc_seasonal_dbc_production (created_time, category, item_description, item_code, batch_received, quantity_received, received_date, report_date, date_created, posted_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $db->prepare($queryDataInsert);
...
if($mode == 'getActualYield'){
    ...
    $query ="SELECT * FROM dbc_seasonal_itemlist WHERE item_code=?";  
    $stmt = $db->prepare($query);
...
if ($mode == 'voiddbc') {
    ...
    $itemdescription = $_POST['itemdescription'];
    $qty = $_POST['qty'];
    $user = $_SESSION['dbc_seasonal_appnameuser'];

   
    $updateQueryProduction = "UPDATE dbc_seasonal_dbc_production
    ...
    if ($stmtProduction) {
        ...
        if ($stmtProduction->execute()) {
            ...
            echo '<script>
            		Check_Permissions("p_view",openMenuGranted,"dbc_seasonal_receiving","DBC Receiving");
            		app_alert("System Message", "Void item '.$itemdescription.'", "success");
            	</script>';
            	
        } else {
            echo '<script>app_alert("System Message", "Error updating status in dbc_seasonal_dbc_production", "error");</script>';
        }
        $stmtProduction->close();
    } else {
        echo '<script>app_alert("System Message", "Error preparing update query for dbc_seasonal_dbc_production", "error");</script>';
    }
...
if ($mode == 'receivedbc') {
    ...
    $updateQueryStock = "UPDATE dbc_seasonal_inventory_stock
        ...
        if ($stmtStock->execute()) {
            ...
            if ($stmtProduction) {
                ...
                if ($stmtProduction->execute()) {
                    ...
                    $queryDataUpdate = "INSERT INTO dbc_seasonal_fgts_pcount (item_code,category,item_description,trans_date,pcount,receiving_detail_id,remarks,date_created,posted_by) VALUES ('$itemcode','$category','$itemdescription','$reportdate','$pcount','$id','$remarks','$date_time','$user')";
					if ($db->query($queryDataUpdate) === TRUE)
					{
						echo '<script>
								searchMe();
            ...
            echo '<script>app_alert("System Message", "Error updating item in dbc_seasonal_inventory_stock", "error");</script>';
        }
        $stmtStock->close();
    } else {
        echo '<script>app_alert("System Message", "Error preparing update query for dbc_seasonal_inventory_stock", "error");</script>';
    }
...
if($mode == 'reopenclosedorderrequest')
{
	...
	$query = "SELECT * FROM dbc_seasonal_request WHERE control_no='$control_no' AND flag=0"; 
	...
	/*
	$query = "SELECT * FROM dbc_seasonal_inventory_records WHERE year = ? AND month = ?";
...
if($mode == 'grandfdsrequest')
{
	...
	$queryDataUpdate = "UPDATE dbc_seasonal_request SET granted_by='$app_user',granted_date='$date_time', flag=1 WHERE id='$rowid'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
...
if($mode == 'reopenorderrequest')
{
	...
	if ( $results->num_rows > 0 ) 
	{
		...
		$queryInsert = "INSERT INTO dbc_seasonal_request ($column) VALUES ($insert)";
...
if($mode == 'updateleadtime')
{
	...
	$queryDataUpdate = "UPDATE dbc_seasonal_inventory_leadtime SET average_leadtime='$minLT', max_leadtime='$maxLT' WHERE leadtime_id=1";
	if ($db->query($queryDataUpdate) === TRUE)
	{
...
{
	$_SESSION['DBC_SEASONAL_REPORT_BRANCH'] = $_POST['branch'];
}
if($mode == 'loadbranch')
{
...
if($mode == 'setbranch')
{
	$_SESSION['DBC_SEASONAL_BRANCH'] = $_POST['branch'];
}
if($mode == 'setcluster')
{
	$_SESSION['DBC_SEASONAL_CLUSTER'] = $_POST['cluster'];
}
if($mode == 'undopcount')
{
	...
	$itemcode = $_POST['itemcode'];
	$elemid = $_POST['elemid'];
	$sqlQueryStk = "SELECT * FROM dbc_seasonal_inventory_stock WHERE item_code='$itemcode' AND stock_before_pcount_date='$transdate'";
	echo $sqlQueryStk;
    ...
    if ($stkResults->num_rows > 0)
    {
		...
		$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_in_hand='$stock_before_pcount' WHERE inventory_id='$rowid'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
	    	$qDelete = "DELETE FROM dbc_seasonal_inventory_pcount WHERE trans_date='$transdate' AND item_code='$itemcode'";
...
if($mode == 'saveinvsetup')
{
	...
	$query = "SELECT * FROM dbc_seasonal_inventory_pcount WHERE trans_date='$transdate' AND item_code='$itemcode'";
	$results = $db->query($query);			
    if($results->num_rows > 0)
    {
    	$update = "`item_code`='$itemcode',`category`='$category',`item_description`='$itemname',`uom`='$uom',`trans_date`='$transdate',`p_count`='$phycount'";
    	$queryDataUpdate = "UPDATE dbc_seasonal_inventory_pcount SET $update WHERE trans_date='$transdate' AND item_code='$itemcode'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		...
		$queryInsert = "INSERT INTO dbc_seasonal_inventory_pcount ($column) VALUES ($insert)";
		if ($db->query($queryInsert) === TRUE)
		{
			executeInventory($itemcode,$category,$itemname,$uom,$function,$phycount,$date_time,$date,$app_user,$db);
		} else {
...
function executeInventory($itemcode,$category,$itemname,$uom,$function,$phycount,$date_time,$date,$app_user,$db)
{
	...
	$sqlQueryStk = "SELECT * FROM dbc_seasonal_inventory_stock WHERE item_code='$itemcode'";
    $stkResults = mysqli_query($db, $sqlQueryStk);
    if ($stkResults->num_rows > 0)
    {
       	while($ROWS = mysqli_fetch_array($stkResults))  
		...
		{	
			$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_before_pcount_date='$date', stock_in_hand='$phycount', date_updated='$date_time',updated_by='$app_user' WHERE item_code='$itemcode'";
	        if ($db->query($queryDataUpdate) === TRUE) {
	        } else {
	            echo $db->error;
	        }
		...
		{
			$queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_before_pcount_date='$date',stock_before_pcount='$value_before_pcount', stock_in_hand='$phycount', date_updated='$date_time',updated_by='$app_user' WHERE item_code='$itemcode'";
	        if ($db->query($queryDataUpdate) === TRUE) {
	        } else {
	            echo $db->error;
	        }
    ...
    else
    {
        ...
        $queryInsert = "INSERT INTO dbc_seasonal_inventory_stock ($column) VALUES ($insert)";
        if ($db->query($queryInsert) === TRUE) {
        } else {
            echo $db->error;
        } 
...
if($mode == 'saveexpdate')
{
    ...
    if($results->num_rows > 0)
    {
    	$queryDataUpdate = "UPDATE dbc_seasonal_inventory_pcount SET expiration_date='$expdate' WHERE trans_date='$transdate' AND item_code='$itemcode'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			echo $db->error;
...
if($mode == 'saveremarks')
{
    ...
    if($results->num_rows > 0)
    {
    	$queryDataUpdate = "UPDATE dbc_seasonal_inventory_pcount SET remarks='$remarks' WHERE trans_date='$transdate' AND item_code='$itemcode'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			echo $db->error;
...
if($mode == 'savepcount')
{
    ...
    if($results->num_rows > 0)
    {
    	$queryDataUpdate = "UPDATE dbc_seasonal_inventory_pcount SET p_count='$phycount' WHERE trans_date='$transdate' AND item_code='$itemcode'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
		} else {
			echo $db->error;
...
if($mode == 'setintransit')
{
	...
	{
	    $date = date("Y-m-d");
	    $queryDataUpdate = "UPDATE dbc_seasonal_order_request SET order_transit=1, status='In-Transit',order_transit_date=? WHERE control_no=? ORDER BY request_id DESC";
	    $stmt = $db->prepare($queryDataUpdate);
	...
	{
	    $col = "day_" . $day;	    
	    $queryDataUpdate = "UPDATE dbc_seasonal_inventory_records SET $col=? WHERE item_code=? AND year=? AND month=?";
	    $stmt = $db->prepare($queryDataUpdate);
	...
	function insertToInventory($year, $month, $day, $item_code, $unit_price, $item_description, $actual_quantity, $control_no, $db)
	{
		...
		$queryInsert = "INSERT INTO dbc_seasonal_inventory_records ($column) VALUES ($insert)";
		if ($db->query($queryInsert) === TRUE)
		{
	        updateLogistics($control_no, $db);
	        print_r('
	...
	{
	    $col = "day_" . $day;    
	    $QUERYRECORDS = "SELECT * FROM dbc_seasonal_inventory_records WHERE item_code=? AND year=? AND month=?";
	...
	function updateStock($item_code,$new_stock,$db)
	{
	    $queryDataUpdate = "UPDATE dbc_seasonal_inventory_stock SET stock_in_hand=? WHERE item_code=?";
	    $stmt = $db->prepare($queryDataUpdate);
	...
	$QUERY = "SELECT * FROM dbc_seasonal_branch_order WHERE control_no=?";
	$stmt = $db->prepare($QUERY);
...
if($mode == 'savelogisticinfo')
{
	...
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request SET delivery_date=?, delivery_driver=?, plate_number=? WHERE control_no=?";
	$stmt = $db->prepare($queryDataUpdate);
...
if($mode == 'forwardtologistics')
{
	...
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request SET logistics=1, dr_number='$dr_number' WHERE control_no='$control_no'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
		$queryDataUpdate = "UPDATE dbc_seasonal_form_numbering SET dr_number='$new_dr_no' WHERE id=1";
		if ($db->query($queryDataUpdate) === TRUE)
...
if($mode == 'preparatorremarks')
{
	...
	$sqlQuery = "SELECT * FROM dbc_seasonal_branch_order_remarks WHERE control_no='$control_no'";
	$results = mysqli_query($db, $sqlQuery);
	if ( $results->num_rows > 0 ) 
    {
		$queryDataUpdate = "UPDATE dbc_seasonal_branch_order_remarks SET preparator_remarks='$remarks' WHERE control_no='$control_no'";
		if ($db->query($queryDataUpdate) === TRUE)
		{
	...
	{	
		$queryInsert = "INSERT INTO dbc_seasonal_branch_order_remarks (`preparator_remarks`,`control_no`) VALUES ('$remarks','$control_no')";
		if ($db->query($queryInsert) === TRUE)
		{
		} else {
			echo $db->error;
...
*/	
	$queryDataUpdate = "UPDATE dbc_seasonal_branch_order SET wh_quantity='$actual_quantity', actual_quantity='$actual_quantity' WHERE id='$rowid'";
	if ($db->query($queryDataUpdate) === TRUE)
	{
		print_r('
			<script>
...
if($mode == 'preparebranchorder')
{
	...
	$module = $_POST['module'];
	$control_no = $_POST['control_no'];
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request SET order_preparing=1, order_preparing_by='$app_user', order_preparing_date='$date' WHERE control_no='$control_no' AND order_preparing=0";
	if ($db->query($queryDataUpdate) === TRUE)
	{
...
if($mode == 'receivebranchorder')
{
	...
	$control_no = $_POST['control_no'];
	$module = $_POST['module'];
	$queryDataUpdate = "UPDATE dbc_seasonal_order_request SET order_received=1, order_received_by='$app_user', order_received_date='$date' WHERE control_no='$control_no' AND order_received=0";
	if ($db->query($queryDataUpdate) === TRUE)
	{
...
if($mode == 'recevingstocks')
{
	...
	$sqlQuery = "SELECT * FROM dbc_seasonal_itemlist $q";
	$results = mysqli_query($db, $sqlQuery);    
	echo '<ul class="searchlist">';
    if ( $results->num_rows > 0 ) 
    {
