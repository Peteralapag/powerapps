...
$_SESSION['DBC_SEASONAL_ITEMCATEGORY'] = $_POST['category'];
if(isset($_SESSION['DBC_SEASONAL_CLUSTER']))
{
	$cluster = $_SESSION['DBC_SEASONAL_CLUSTER'];
	if(isset($_SESSION['DBC_SEASONAL_BRANCH']))
	{
		$thebranch = $_SESSION['DBC_SEASONAL_BRANCH'];
	} else {
		$thebranch = '';
	}
} else {
...
if(isset($_SESSION['DBC_SEASONAL_YEARS']))
{
	$theyears = $_SESSION['DBC_SEASONAL_YEARS'];
} else {
	$theyears = date("Y");	
}
if(isset($_SESSION['DBC_SEASONAL_MONTHS']))
{
	$themonths = $_SESSION['DBC_SEASONAL_MONTHS'];
} else {
	$themonths = date("m");	
}
if(isset($_SESSION['DBC_SEASONAL_DAYS']))
{
	$thedays = $_SESSION['DBC_SEASONAL_DAYS'];
} else {
	$thedays = date("d");	
}
...
function setBranch(branch)
{
	...
	var mode = 'setbranch';
	$.post("./Modules/DBC_Seasonal_Management/actions/actions.php", { mode: mode, cluster: cluster, branch: branch },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
...
{
	var mode = 'setcluster';
	$.post("./Modules/DBC_Seasonal_Management/actions/actions.php", { mode: mode, cluster: cluster },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
		loadBranch(cluster);
...
function loadBranch(cluster)
{
	...
	$.post("./Modules/DBC_Seasonal_Management/actions/actions.php", { mode: mode, branch: branch, cluster: cluster },
	function(data) {		
		$('#branches').html(data);
		rms_reloaderOff();
	}); 
...
function GenerateExcelDaily()
{
		...
		$("#genxcel_page").attr("src", "./Modules/DBC_Seasonal_Management/reporting/generate_excel_daily.php?years=" + theyears + "&months=" + themonths + "&days=" + thedays);
		$("#genxcel").show();
...
function inventoryPcount()
{
	...
	$.post("./Modules/DBC_Seasonal_Management/reporting/inventory_setup.php", { theyears: theyears, themonths: themonths, thedays: thedays, location: location },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
...
function loadDataNow()
{
	...
	$.post("./Modules/DBC_Seasonal_Management/reporting/inventory_sheet_data.php", { theyears: theyears, themonths: themonths, thedays: thedays },
	function(data) {		
		$('#sheetdata').html(data);
		rms_reloaderOff();
	}); 
