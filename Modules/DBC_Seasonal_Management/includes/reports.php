...
if(isset($_SESSION['DBC_SEASONAL_TABLE']))
{
	$dbc_table = $_SESSION['DBC_SEASONAL_TABLE'];
} else {
	$dbc_table = 'dbc_seasonal_supplier';
}
?>
<style>
...
function GenerateExcel(table)
{
	if(sessionStorage.stringKo === undefined || sessionStorage.stringKo === null)
	{
		...
		$("#genxcel_page").attr("src", "./Modules/DBC_Seasonal_Management/reporting/generate_excel.php?table=" + table + "&columns=" + sessionStorage.stringKo);
		$("#genxcel").show();
...
$(function()
{
	if('<?php echo $dbc_table; ?>' == 'dbc_seasonal_inventory_records')
	{
		$('#dateinputs').show();
	} else {
		$('#dateinputs').hide();
...
{
	var limit = $('#limit').val();
	$.post("./Modules/DBC_Seasonal_Management/reporting/inventory_report.php", { table: table, datefrom: datefrom, dateto: dateto },
	function(data) {		
		$('#smnavdata').html(data);
	});
}
function loadModule(table)
{
	var limit = $('#limit').val();
	 ...
	 {
		$.post("./Modules/DBC_Seasonal_Management/reporting/show_report.php", { table: table },
		function(data) {		
			$('#smnavdata').html(data);
		});	 	
	 } else {
 	 	var datefrom = $('#date_from').val();
	 	var dateto = $('#date_to').val();
		$.post("./Modules/DBC_Seasonal_Management/reporting/inventory_report.php", { limit: limit },
		function(data) {		
			$('#smnavdata').html(data);
		});
	}
