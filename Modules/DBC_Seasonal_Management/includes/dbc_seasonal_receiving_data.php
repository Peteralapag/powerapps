...
<?PHP
    ...
    if ( $results->num_rows > 0 ) 
    {
    	...
    	while($RECVROW = mysqli_fetch_array($results))  
		{
			...
			$pcount = $status == 'No'? '' : $function->GetDataByRecevingId('pcount','dbc_seasonal_fgts_pcount',$id,$db);
			$remarks = $status == 'No'? '' : $function->GetDataByRecevingId('remarks','dbc_seasonal_fgts_pcount',$id,$db);
...
function receiveThis(n,id,itemcode,category,itemdescription,qty,reportdate,shift)
{
	...
	rms_reloaderOn('Loading...');
	$.post("./Modules/DBC_Seasonal_Management/actions/actions.php", { mode: mode, id: id, itemcode: itemcode, category: category, itemdescription: itemdescription, qty: qty, reportdate: reportdate, shift: shift, pcount: pcount, remarks: remarks },
	function(data) {		
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});		
