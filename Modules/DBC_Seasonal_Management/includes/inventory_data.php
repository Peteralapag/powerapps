...
{
	$_SESSION['DBC_SEASONAL_ITEM_LOCATION'] = $_POST['location'];
} else {
	unset($_SESSION['DBC_SEASONAL_ITEM_LOCATION']);
}
if(isset($_POST['limit']) && $_POST['limit'] != '')
{
	$_SESSION['DBC_SEASONAL_SHOW_LIMIT'] = $_POST['limit'];
} else {
	unset($_SESSION['DBC_SEASONAL_SHOW_LIMIT']);
}
if($_POST['limit'] != '')
{
...
if(isset($_POST['search']))
{
	...
	if(isset($_POST['location']) && $_POST['location'] != '')
	{
		$q = "WHERE dbc_seasonal_itemlist.item_location='$location' AND  (dbc_seasonal_inventory_stock.item_code LIKE '%$search%' OR dbc_seasonal_inventory_stock.item_description LIKE '%$search%')";
	} else {	
		$q = "WHERE dbc_seasonal_inventory_stock.item_code LIKE '%$search%' OR dbc_seasonal_inventory_stock.item_description LIKE '%$search%'";
	}
	...
	if(isset($_POST['location']) && $_POST['location'] != '')
	{
		$search = $_POST['search'];	
		$location = $_POST['location'];
		$q = "WHERE dbc_seasonal_itemlist.item_location='$location'";
	} else {
		$q = "ORDER BY dbc_seasonal_inventory_stock.supplier_id DESC";
	}
}
?>
<style>
...
<?PHP
	$sqlQuery = "
		SELECT * FROM dbc_seasonal_inventory_stock 
        INNER JOIN dbc_seasonal_itemlist ON dbc_seasonal_inventory_stock.item_code = dbc_seasonal_itemlist.item_code
        $q $limit
    ";
    ...
    if ( $results->num_rows > 0 ) 
    {
    	...
    	while($INVROW = mysqli_fetch_array($results))  
		{
			...
			$sqlQueryRecords = "SELECT * FROM dbc_seasonal_inventory_records WHERE year='$year' AND month='$month' AND item_code='$item_code'";
...
function itemDetails(itemcode,item)
{
	...
	$.post("./Modules/DBC_Seasonal_Management/apps/inventory_item_details.php", { item_code: itemcode, item: item },
	function(data) {		
