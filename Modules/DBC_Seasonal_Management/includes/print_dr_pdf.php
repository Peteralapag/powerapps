...
$page = '';
$sqlPageCnt = "SELECT * FROM dbc_seasonal_branch_mrs_transaction WHERE request_id='$rowid' AND cancelled=0 AND quantity > 0";
$result = mysqli_query($db, $sqlPageCnt);
$totalRows = mysqli_num_rows($result);

$itemsPerDiv = 10; // Items per page
...
<div style="height:13cm;border-bottom-style:;position:relative;last-child:border:0;font-family:Arial, sans-serif">
    <div style="width: 8in;height:5in;margin-left:0.25in;margin-top: 0.25in;page-break-inside: avoid">
        <div>
        ...
        <div class="dr-info">            
            <table style="width: 100%">
                <tr style="font-family: Verdana, Geneva, Tahoma, sans-serif;font-size:16px">
                    ...
                    <td style="border-bottom:1px solid #232323;width: 1in;color:red;text-align:center;padding:2px">'.$function->GetOrderStatusSecondTable($rowid, 'dr_number', $db).'</td>
                </tr>
                <tr style="font-family: Verdana, Geneva, Tahoma, sans-serif;font-size:16px">
                    <td style="width:100px;font-weight:600;white-space:nowrap;padding:2px">Branch: </td>
                    <td style="border-bottom:1px solid #232323;padding:2px">'.$function->GetOrderStatusSecondTable($rowid, 'branch', $db).'</td>
                    <td style="width:1in;text-align:right;font-weight:600;padding:2px">Ref No.: </td>
                    <td style="border-bottom:1px solid #232323;width: 1in;text-align:center;padding:2px">'.$control_no.'</td>
                </tr>
            </table>            
        </div>
    ...
    $sqlQuery = "SELECT * FROM dbc_seasonal_branch_mrs_transaction WHERE request_id='$rowid' AND cancelled=0 AND quantity > 0 LIMIT $startFrom, $itemsPerDiv";
    ...
    while ($ROWS = mysqli_fetch_array($results)) {
        ...
        $actual_quantity = $ROWS['wh_quantity'];
			...
			<table style="width: 100%">
				<tr>
					...
					<td colspan="1" style="text-align:center;font-size:14px;font-family:Lucida Sans, Geneva, Verdana, sans-serif">
						<div style="margin:0 auto;width:180px"></div>
					</td>
				</tr>
				<tr>
				...
				<tr style="font-size:14px;font-family:Lucida Sans, Geneva, Verdana, sans-serif">
					...
					<td style="width:1.80in;border-bottom:1px solid #232323;text-align:center">'.$function->GetOrderStatusSecondTable($rowid,'delivery_driver',$db).'</td>
					<td>&nbsp;</td>
					<th style="width:0.50in">Plate No.</th>
					<td style="border-bottom:1px solid #232323;width:0.75in;text-align:center">'.$function->GetOrderStatusSecondTable($rowid,'plate_number',$db).'</td>
					<td>&nbsp;</td>
