	...
	<table style="width: 100%" class="lamesako">
		<thead>
			<tr>
				...
				<th title="UNIT OF MEASURE" style="width:100px">UNITS (UOM)</th>
				<th title="REQUESTED QUANTITY" style="width:100px">REQUESTED QTY.</th>
				<!--th title="ACTUAL QUANTITY" style="width:100px">ACTUAL QTY.</th-->
				<!--th title="ACTIONS" style="width:70px">ACTIONS</th-->
				<!--th title="DELIVERY STATUS">DEL. STAT.</th-->
				<th title="REMAINING BALANCE" style="text-align:center">REMAINING BALANCE</th>
			</tr>
		</thead>
		<tbody>
<?php
//	$sqlQuery = "SELECT * FROM dbc_seasonal_branch_order WHERE control_no='$control_no'";
	
	$sqlQuery = "
	    SELECT dbo.*, dil.ordered 
	    FROM dbc_seasonal_branch_order dbo
	...
	{ 
		...
		if($function->GetItemStock($item_code,$db) <= 0 || $function->GetItemStock($item_code,$db) <= 0.00 || $finalize == 0)
		{
			$stock_text = 'disabled';
			$actual_quantity = 0;
			$actual_quantity = '';
		...
		$remainingqty = $quantity - $function->getSumRemainingBalance($controlno, $item_code, $db);
?>		
			<tr>
				...
				<td style="text-align:center"><?php echo number_format($remainingqty,2)?></td>
...
<?php } ?>
			<tr class="remarks-footer">
				...
				<td colspan="6"><textarea type="text" class="form-control form-control-sm" placeholder="Preparator Remarks" onkeyUp="prepRemarks(this.value,'<?php echo $control_no; ?>')"><?php echo $function->getOrderRemarks($control_no,'preparator_remarks',$db); ?></textarea></td>
			</tr>
<?php } else { $basket = 0; ?>    			
			<tr>
				<td colspan="8" style="padding:8px;text-align:center;font-weight:600" class="bg-warning color-white">THE ORDER BASKET FROM THIS BRANCH IS EMPTY.</td>
			</tr>
<?php } ?>
...
<div class="closedform">
	<img src="../images/closed-stamp.png" style="opacity: 0.3"></div>
</div>
<div id="results"></div>
<script>
...
function prepRemarks(value,controlno)
{
	...
	$.post("./Modules/DBC_Seasonal_Management/actions/actions.php", { mode: mode, control_no: controlno, remarks: value },
	function(data) {		
		$('#results').html(data);
	});
}
function changeAQ(elemid,rowid,quantity,item_code,transdate)
{
	...
	setTimeout(function()
	{
		$.post("./Modules/DBC_Seasonal_Management/actions/actions.php", { mode: mode, rowid: rowid, actual_quantity: actual_quantity, control_no: control_no, item_code: item_code, transdate: transdate },
		function(data) {		
			$('#results').html(data);
