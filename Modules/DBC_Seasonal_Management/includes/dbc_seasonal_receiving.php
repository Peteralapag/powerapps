...
if(isset($_SESSION['DBC_SEASONAL_ORD']))
{
	$ordd = $_SESSION['DBC_SEASONAL_ORD'];
} else {
	$ordd = "Process Order";
}
if(isset($_SESSION['dbc_seasonal_user_recipient']))
{
	if($user_level >= 60)
	{
		$user_recipient = $_SESSION['dbc_seasonal_user_recipient'];
		$admin = 1;			
	} else {
		$admin = 0;
		$user_recipient = $_SESSION['dbc_seasonal_user_recipient'];
...
$ord = '';
$imagelink = $_SERVER['DOCUMENT_ROOT']."/Modules/DBC_Seasonal_Management/images/company_logo.png";


$trans_date = $_SESSION['DBC_SEASONAL_TRANSDATE'];
?>
<style>
...
function openpendingapproval(){
	...
	$.post("./Modules/DBC_Seasonal_Management/apps/dbc_pending_approval.php", {  },
	function(data) {		
...
function viewSummary(){
	...
	$.post("./Modules/DBC_Seasonal_Management/includes/dbc_summary.php", { },
	function(data) {		
		$('#formmodal_page').html(data);
		$('#formmodal').show();
	});	
	...
	$.post("./Modules/DBC_Seasonal_Management/includes/dbc_summary_data.php", { transdate: transdate },
	function(data) {
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
function searchMe(){
	...
	rms_reloaderOn('Loading...');
	$.post("./Modules/DBC_Seasonal_Management/includes/dbc_seasonal_receiving_data.php", { transdate: transdate },
	function(data) {
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
function getRecipientData()
{
	...
	var ord = $('#ord').val();
	rms_reloaderOn('Loading...');	
	$.post("./Modules/DBC_Seasonal_Management/includes/dbc_seasonal_receiving_data.php", { limit: limit, recipient: recipient, ord: ord },
	function(data) {		
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
$(function()
{
	...
	$('#search').keyup(function()
	{
		...
		$.post("./Modules/DBC_Seasonal_Management/includes/dbc_seasonal_receiving_data.php", { limit: limit, search: search, recipient: recipient, ord: ord },
		function(data) {
			$('#smnavdata').html(data);
		});
