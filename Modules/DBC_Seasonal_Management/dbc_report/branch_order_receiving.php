...
{
	$year = $_SESSION['DBC_SEASONAL_YEAR'];
} else {
	$year = date("Y");
}
if(isset($_SESSION['DBC_SEASONAL_MONTH']))
{
	$month = $_SESSION['DBC_SEASONAL_MONTH'];
} else {
	$month = date("m");
}
if(isset($_SESSION['DBC_SEASONAL_WEEK']))
{
	$week = $_SESSION['DBC_SEASONAL_WEEK'];
} else {
	$week = 1;
}
?>
...
function searchItem() {
	...
	$.post("./Modules/DBC_Seasonal_Management/dbc_report/search_processor.php", {
		...
		if (data.branch_page) {
			loadCount++;
			$("#iwd").load("./Modules/DBC_Seasonal_Management/dbc_report/" + data.branch_page, {
				selectedbranch: branchInput,
				dateFrom: dateFrom,
				dateTo: dateTo
			}, checkIfAllLoaded);
		...
		if (data.cluster_page) {
			loadCount++;
			$("#iwd").load("./Modules/DBC_Seasonal_Management/dbc_report/" + data.cluster_page, {
		...
		if (data.allbranch_page) {
			loadCount++;
			$("#iwd").load("./Modules/DBC_Seasonal_Management/dbc_report/" + data.allbranch_page, {
