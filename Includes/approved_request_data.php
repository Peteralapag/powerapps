...
$ANRQUERYCNT = "SELECT * FROM tbl_app_request WHERE approved=1 AND executed=1";
$ANRRESULTS = mysqli_query($db, $ANRQUERYCNT); 
if ( $ANRRESULTS->num_rows > 0 ) 
{
	$anr_cnt = $ANRRESULTS->num_rows; 
} else {
	$anr_cnt = 0;
}
...
<table style="width: 100%" class="table">
	<thead>
		<tr>
			<th style="text-align:center" class="bg-warning">(<?php echo $anr_cnt; ?>) APPROVED REQUEST</th>
		</tr>
	</thead>
	<tbody>
<?php
$sqlNotifs = "SELECT * FROM tbl_app_request WHERE approved=1 AND executed=1 ORDER BY id DESC";
...
<?php } } else { ?>
		<tr>
			<td style="text-align:center">No Approved Request</td>
		</tr>
<?PHP } mysqli_close($db); ?>
	</tbody>
</table>
